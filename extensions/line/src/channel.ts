import {
  ...
  type ZoidbergBotConfig,
  type LineConfig,
  type LineChannelData,
  type ResolvedLineAccount,
} from "zoidbergbot/plugin-sdk";
...
export const linePlugin: ChannelPlugin<ResolvedLineAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      ...
      await line.pushMessageLine(id, "ZoidbergBot: your access has been approved.", {
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      return {
        ...
        approveHint: "zoidbergbot pairing approve line <code>",
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      const nextCfg = { ...cfg } as ZoidbergBotConfig;
      const lineConfig = (cfg.channels?.line ?? {}) as LineConfig;
