...
// NOTE: This extension is intended to be bundled with ZoidbergBot.
// When running from source (tests/dev), ZoidbergBot internals live under src/.
...
export function createLlmTaskTool(api: ZoidbergBotPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via zoidbergbot.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-llm-task-"));
