...
final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    ...
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        guard let name = ZoidbergBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId =
        ...
        let contextJSON = ZoidbergBotCanvasA2UIAction.compactJSON(userAction["context"])

        // Token-efficient and unambiguous. The agent should treat this as a UI event and (by default) update Canvas.
        let messageContext = ZoidbergBotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
        ...
        let text = ZoidbergBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        Task { [weak webView] in
            ...
            await MainActor.run {
                ...
                let js = ZoidbergBotCanvasA2UIAction.jsDispatchA2UIActionStatus(
                    actionId: actionId,
                    ok: result.ok,
                    error: result.error)
    ...
    // Formatting helpers live in ZoidbergBotKit (`ZoidbergBotCanvasA2UIAction`).
}
