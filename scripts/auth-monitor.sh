...
#
# Suggested cron: */30 * * * * /home/admin/zoidbergbot/scripts/auth-monitor.sh
#
...
STATE_FILE="$HOME/.zoidbergbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ZoidbergBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        ...
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ZoidbergBot to $NOTIFY_PHONE..."
            zoidbergbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: ZoidbergBot Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ZoidbergBot is down. Run: ssh l36 '~/zoidbergbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
