...
IMAGE_NAME="zoidbergbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/zoidbergbot-bin:$PATH"
  mkdir -p /tmp/zoidbergbot-bin

  cat > /tmp/zoidbergbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/zoidbergbot-bin/systemctl

  cat > /tmp/zoidbergbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/zoidbergbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/zoidbergbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/zoidbergbot/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/zoidbergbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/zoidbergbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/zoidbergbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
