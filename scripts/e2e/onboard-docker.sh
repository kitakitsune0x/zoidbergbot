...
IMAGE_NAME="zoidbergbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/zoidbergbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/zoidbergbot-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/zoidbergbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/zoidbergbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/zoidbergbot-e2e-$1.XXXXXX"
  }
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.zoidbergbot/zoidbergbot.json" <<'"'"'JSON'"'"'
{
