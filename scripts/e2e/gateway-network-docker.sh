...
IMAGE_NAME="zoidbergbot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="zoidbergbot-net-e2e-$$"
GW_NAME="zoidbergbot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ZOIDBERGBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "ZOIDBERGBOT_SKIP_CHANNELS=1" \
	  -e "ZOIDBERGBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "ZOIDBERGBOT_SKIP_CRON=1" \
	  -e "ZOIDBERGBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
