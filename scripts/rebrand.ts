...
const CONTENT_REPLACEMENTS: Array<{ from: string; to: string }> = [
  ...
  { from: "openclaw.ai", to: "zoidbergbot.ai" },
  {
    ...
    to: "github.com/zoidbergbot/zoidbergbot",
  },
  ...
  { from: '".openclaw\\\\', to: '".zoidbergbot\\\\' },
  { from: "`.openclaw`", to: "`.zoidbergbot`" },
  // Template literal paths (e.g., `.openclaw${suffix}`)
  { from: "`.openclaw${", to: "`.zoidbergbot${" },
  ...
  // Tagline
  {
    from: "All your chats, one OpenClaw.",
    to: "All your chats, one ZoidbergBot.",
  },
...
const FILE_RENAMES: Array<{ from: string; to: string }> = [
  ...
  {
    from: "openclaw-auth-monitor.service",
    to: "zoidbergbot-auth-monitor.service",
  },
  { from: "openclaw-auth-monitor.timer", to: "zoidbergbot-auth-monitor.timer" },
  { from: "openclaw.plugin.json", to: "zoidbergbot.plugin.json" },
];
...
async function processFile(filePath: string): Promise<void> {
  ...
  try {
    ...
    if (changeCount > 0) {
      ...
      if (DRY_RUN) {
        console.log(
          `[DRY-RUN] Would modify: ${filePath} (${changeCount} replacements)`
        );
      } else {
...
async function renameDirIfNeeded(dirPath: string): Promise<string> {
  ...
  for (const { from, to } of DIR_RENAMES) {
    if (name === from) {
      ...
      if (DRY_RUN) {
        console.log(
          `[DRY-RUN] Would rename directory: ${dirPath} -> ${newPath}`
        );
      } else {
