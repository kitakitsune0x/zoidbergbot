...
read_when:
  - Setting up Twitch chat integration for ZoidbergBot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ZoidbergBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
zoidbergbot plugins install @zoidbergbot/twitch
```

Local checkout (when running from a git repo):

```bash
zoidbergbot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ZOIDBERGBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "zoidbergbot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ZOIDBERGBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
ZOIDBERGBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
