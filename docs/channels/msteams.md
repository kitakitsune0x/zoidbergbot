...
```bash
zoidbergbot plugins install @zoidbergbot/msteams
```

Local checkout (when running from a git repo):

```bash
zoidbergbot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
ZoidbergBot will offer the local install path automatically.
...
3. Configure ZoidbergBot with those credentials.
...
- Talk to ZoidbergBot via Teams DMs, group chats, or channels.
...
- On startup, ZoidbergBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.zoidbergbot/zoidbergbot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring ZoidbergBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `zoidbergbot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `zoidbergbot plugins install @zoidbergbot/msteams`
   - From a local checkout: `zoidbergbot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "ZoidbergBot in Teams", "full": "ZoidbergBot in Teams" },
...
ZoidbergBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, ZoidbergBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ZoidbergBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

ZoidbergBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `zoidbergbot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.zoidbergbot/msteams-polls.json`.
...
```bash
zoidbergbot message send --channel msteams \
...
zoidbergbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
zoidbergbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
zoidbergbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
zoidbergbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
