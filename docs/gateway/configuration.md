---
summary: "All configuration options for ~/.zoidbergbot/zoidbergbot.json with examples"
read_when:
...
ZoidbergBot reads an optional **JSON5** config from `~/.zoidbergbot/zoidbergbot.json` (comments + trailing commas allowed).

If the file is missing, ZoidbergBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.zoidbergbot/workspace`). You usually only need a config to:
...
ZoidbergBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `zoidbergbot doctor`, `zoidbergbot logs`, `openclaw health`, `zoidbergbot status`, `openclaw service`, `zoidbergbot help`).
- Run `zoidbergbot doctor` to see the exact issues.
- Run `zoidbergbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `zoidbergbot config set`. Keep a backup of `~/.zoidbergbot/zoidbergbot.json`.
...
```bash
zoidbergbot gateway call config.get --params '{}' # capture payload.hash
zoidbergbot gateway call config.apply --params '{
...
```bash
zoidbergbot gateway call config.get --params '{}' # capture payload.hash
zoidbergbot gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.zoidbergbot/workspace" },
    list: [
      {
        id: "main",
...
```json5
// ~/.zoidbergbot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.zoidbergbot/workspace" }],
}
```
...
{ "$include": "/etc/zoidbergbot/base.json5" } // absolute
...
```json5
// ~/.zoidbergbot/clients/mueller/agents.json5
[
...
```json5
// ~/.zoidbergbot/clients/mueller/broadcast.json5
{
...
ZoidbergBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ZoidbergBot runs your login shell and imports only the missing expected keys (never overrides).
...
- `ZOIDBERGBOT_LOAD_SHELL_ENV=1`
- `ZOIDBERGBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${ZOIDBERGBOT_GATEWAY_TOKEN}",
    },
  },
}
```
...
ZoidbergBot stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.zoidbergbot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.zoidbergbot/credentials/oauth.json` (or `$ZOIDBERGBOT_STATE_DIR/credentials/oauth.json`)
...
- `~/.zoidbergbot/agent/*` (migrated by `zoidbergbot doctor` into `~/.zoidbergbot/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `ZOIDBERGBOT_OAUTH_DIR`
- Agent dir (default agent root override): `ZOIDBERGBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ZoidbergBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ZoidbergBot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/zoidbergbot/zoidbergbot.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/zoidbergbot/zoidbergbot.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.zoidbergbot/credentials/whatsapp/biz
          // authDir: "~/.zoidbergbot/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `zoidbergbot doctor` into `whatsapp/default`.
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.zoidbergbot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.zoidbergbot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.zoidbergbot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.zoidbergbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.zoidbergbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.zoidbergbot/workspace-home" },
      { id: "work", workspace: "~/.zoidbergbot/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `zoidbergbot.json`).
...
ZoidbergBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["zoidbergbot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
ZoidbergBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `zoidbergbot plugins install @zoidbergbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ZoidbergBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ZoidbergBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ZoidbergBot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/zoidbergbot" } },
}
```
...
When a file exceeds this limit, ZoidbergBot logs a warning and injects a truncated
...
message envelopes). If unset, ZoidbergBot uses the host timezone at runtime.
...
`[zoidbergbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[zoidbergbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[zoidbergbot]"`, ZoidbergBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ZoidbergBot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.zoidbergbot/settings/tts.json",
      elevenlabs: {
...
ZoidbergBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, ZoidbergBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `zoidbergbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in ZoidbergBot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.zoidbergbot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.zoidbergbot/sandboxes",
        docker: {
          image: "zoidbergbot-sandbox:bookworm-slim",
          containerPrefix: "zoidbergbot-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "zoidbergbot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "zoidbergbot-sandbox-browser:bookworm-slim",
          containerPrefix: "zoidbergbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
ZoidbergBot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.zoidbergbot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ZoidbergBot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ZoidbergBot writes/merges a `models.json` into
`~/.zoidbergbot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ZoidbergBot uses
...
- Override the agent config root with `ZOIDBERGBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.zoidbergbot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.zoidbergbot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.zoidbergbot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ZoidbergBot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.zoidbergbot/skills` (workspace skills
...
from `~/.zoidbergbot/extensions`, `<workspace>/.zoidbergbot/extensions`, plus any
...
### `browser` (zoidbergbot-managed browser)

ZoidbergBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for zoidbergbot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (ZoidbergBot.app menubar, or `zoidbergbot gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      zoidbergbot: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/zoidbergbot" }
...
- Examples: `"/ui"`, `"/zoidbergbot"`, `"/apps/zoidbergbot"`.
...
- When a connection comes from one of these IPs, ZoidbergBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `zoidbergbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `ZOIDBERGBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `ZOIDBERGBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ZoidbergBot
...
- ZoidbergBot.app watches `~/.zoidbergbot/zoidbergbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.zoidbergbot/zoidbergbot.json` (or `ZOIDBERGBOT_CONFIG_PATH`) and applies changes automatically.
...
- `~/.zoidbergbot/zoidbergbot.json` (or `ZOIDBERGBOT_CONFIG_PATH`)
...
- `ZOIDBERGBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `zoidbergbot --dev …` → uses `~/.zoidbergbot-dev` + shifts ports from base `19001`
- `zoidbergbot --profile <name> …` → uses `~/.zoidbergbot-<name>` (port via config/env/flags)
...
```bash
ZOIDBERGBOT_CONFIG_PATH=~/.zoidbergbot/a.json \
ZOIDBERGBOT_STATE_DIR=~/.zoidbergbot-a \
zoidbergbot gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.zoidbergbot/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Gmail helper config (used by `zoidbergbot webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "zoidbergbot@gmail.com",
...
- Set `ZOIDBERGBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ZoidbergBot defaults `serve.path` to `/` so
...
Default root: `~/.zoidbergbot/workspace/canvas`  
Default port: `18793` (chosen to avoid the zoidbergbot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__zoidbergbot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__zoidbergbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.zoidbergbot/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `ZOIDBERGBOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.zoidbergbot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.zoidbergbot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.zoidbergbot/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_zoidbergbot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `zoidbergbot` (advertises `zoidbergbot.local`). Override with `ZOIDBERGBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_zoidbergbot-gw._tcp` under `~/.zoidbergbot/dns/` using the configured discovery domain (example: `zoidbergbot.internal.`).
