---
summary: "Quick troubleshooting guide for common ZoidbergBot failures"
read_when:
...
When ZoidbergBot misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `zoidbergbot status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `zoidbergbot status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `zoidbergbot status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `zoidbergbot gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `zoidbergbot channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `zoidbergbot gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `zoidbergbot logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `zoidbergbot status --all` (it redacts tokens). If you paste `zoidbergbot status`, consider setting `ZOIDBERGBOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  zoidbergbot models auth setup-token --provider anthropic
  ```
...
```bash
zoidbergbot models auth paste-token --provider anthropic
zoidbergbot models status
```
...
- Preferred: `zoidbergbot logs --follow`
- File logs (always): `/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$ZOIDBERGBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u zoidbergbot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  zoidbergbot config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  zoidbergbot config set gateway.mode remote
  zoidbergbot config set gateway.remote.url "wss://gateway.example.com"
  ```
...
variables like `DISPLAY` should live in `~/.zoidbergbot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `zoidbergbot doctor`
...
- then run `zoidbergbot sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `zoidbergbot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `zoidbergbot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `zoidbergbot gateway status` and `zoidbergbot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `ZOIDBERGBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `zoidbergbot gateway status` shows a config mismatch**
...
- Fix: rerun `zoidbergbot gateway install --force` from the same `--profile` / `ZOIDBERGBOT_STATE_DIR` you want the service to use.

**If `zoidbergbot gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `zoidbergbot doctor` to update it (or `zoidbergbot gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `ZOIDBERGBOT_GATEWAY_TOKEN`) and restart the service.

**If `zoidbergbot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/zoidbergbot` on disk.
...
Symptoms: `pwd` or file tools show `~/.zoidbergbot/sandboxes/...` even though you
...
ZoidbergBot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `zoidbergbot models list` or
  `zoidbergbot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${ZOIDBERGBOT_CONFIG_PATH:-$HOME/.zoidbergbot/zoidbergbot.json}"
```
...
tail -f "$(ls -t /tmp/zoidbergbot/zoidbergbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
zoidbergbot pairing list <channel>
```
...
```bash
zoidbergbot logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.zoidbergbot/agents/<agentId>/sessions/
```
...
zoidbergbot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
zoidbergbot gateway --verbose
```

If you’re logged out / unlinked:

```bash
zoidbergbot channels logout
trash "${ZOIDBERGBOT_STATE_DIR:-$HOME/.openclaw}/credentials" # if logout can't cleanly remove everything
zoidbergbot channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/zoidbergbot/zoidbergbot-*.log | head -1)" | tail -20
```

### High Memory Usage

ZoidbergBot keeps conversation history in memory.
...
ZoidbergBot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `zoidbergbot doctor` reports every invalid entry.
- `zoidbergbot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `zoidbergbot logs`, `openclaw health`, `zoidbergbot status`, `zoidbergbot gateway status`, and `zoidbergbot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/zoidbergbot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `zoidbergbot models status` (CLI).
...
```bash
zoidbergbot channels login
```

### Build errors on `main` — what’s the standard fix path?

1. `git pull origin main && pnpm install`
2. `zoidbergbot doctor`
...
5. Run `zoidbergbot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. ZoidbergBot scrubs/normalizes tool
schemas in current `main`, but the fix is not in the last release yet (as of
January 13, 2026).

Fix checklist:

1. **Update ZoidbergBot**:
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/zoidbergbot/zoidbergbot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.zoidbergbot.* still works)
```
...
Ensure the global `zoidbergbot` CLI is installed and matches the app version:

```bash
zoidbergbot --version
npm install -g zoidbergbot@<version>
```
...
#   ${ZOIDBERGBOT_CONFIG_PATH:-$HOME/.zoidbergbot/zoidbergbot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
zoidbergbot gateway --verbose
zoidbergbot channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$ZOIDBERGBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.zoidbergbot/logs/...`; profiles use `~/.zoidbergbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u zoidbergbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "ZoidbergBot Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$ZOIDBERGBOT_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$ZOIDBERGBOT_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$ZOIDBERGBOT_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
zoidbergbot gateway status --deep
...
tail -20 /tmp/zoidbergbot/zoidbergbot-*.log
```
...
# zoidbergbot gateway uninstall

trash "${ZOIDBERGBOT_STATE_DIR:-$HOME/.openclaw}"
zoidbergbot channels login         # re-pair WhatsApp
zoidbergbot gateway restart           # or: zoidbergbot gateway
```
...
1. Check logs first: `/tmp/zoidbergbot/` (default: `zoidbergbot-YYYY-MM-DD.log`, or your configured `logging.file`)
