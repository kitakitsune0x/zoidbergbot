---
summary: "ZoidbergBot on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up ZoidbergBot on Oracle Cloud
  - Looking for low-cost VPS hosting for ZoidbergBot
  - Want 24/7 ZoidbergBot on a small server
title: "Oracle Cloud"
---

# ZoidbergBot on Oracle Cloud (OCI)

## Goal

Run a persistent ZoidbergBot Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for ZoidbergBot (especially if you already have an OCI account), but it comes with tradeoffs:
...
zoidbergbot config set gateway.bind loopback

# Require auth for the Gateway + Control UI
zoidbergbot config set gateway.auth.mode token
zoidbergbot doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
zoidbergbot config set gateway.tailscale.mode serve
zoidbergbot config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart zoidbergbot-gateway
```
...
# Check daemon status
systemctl --user status zoidbergbot-gateway
...
```
https://zoidbergbot.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `zoidbergbot security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.zoidbergbot`
- **Security audit:** `zoidbergbot security audit`
...
# Re-authenticate
sudo tailscale up --ssh --hostname=zoidbergbot --reset
```
...
journalctl --user -u zoidbergbot-gateway -n 50
```
...
- `~/.zoidbergbot/` — config, credentials, session data
- `~/.zoidbergbot/workspace/` — workspace (SOUL.md, memory, artifacts)
