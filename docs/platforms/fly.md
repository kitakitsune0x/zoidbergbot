---
title: Fly.io
description: Deploy ZoidbergBot on Fly.io
---

# Fly.io Deployment

**Goal:** ZoidbergBot Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
[env]
  NODE_ENV = "production"
  ZOIDBERGBOT_PREFER_PNPM = "1"
  ZOIDBERGBOT_STATE_DIR = "/data"
...
| `internal_port = 3000`         | Must match `--port 3000` (or `ZOIDBERGBOT_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `ZOIDBERGBOT_STATE_DIR = "/data"` | Persists state on the volume                                                |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set ZOIDBERGBOT_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `ZOIDBERGBOT_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `zoidbergbot.json` where they could be accidentally exposed or logged.
...
**Note:** With `ZOIDBERGBOT_STATE_DIR=/data`, the config path is `/data/zoidbergbot.json`.
...
Or visit `https://my-zoidbergbot.fly.dev/`

Paste your gateway token (the one from `ZOIDBERGBOT_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `ZOIDBERGBOT_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/zoidbergbot.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/zoidbergbot.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/zoidbergbot.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/zoidbergbot.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/zoidbergbot.json"
```
...
**Fix:** Ensure `ZOIDBERGBOT_STATE_DIR=/data` is set in `fly.toml` and redeploy.
