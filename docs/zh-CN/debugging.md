...
当你需要切换一些不常用的设置而又不想编辑 `zoidbergbot.json` 时，这非常方便。

示例：

```
/debug show
/debug set messages.responsePrefix="[zoidbergbot]"
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.zoidbergbot-dev`，并将 Gateway网关默认端口设为 `19001`（派生端口随之偏移）。
...
```bash
pnpm gateway:dev
ZOIDBERGBOT_PROFILE=dev zoidbergbot tui
```

如果你还没有全局安装，可通过 `pnpm zoidbergbot ...` 运行 CLI。
...
1. **配置文件隔离**（全局 `--dev`）
   ...
   - `ZOIDBERGBOT_GATEWAY_PORT=19001`（浏览器/画布端口随之偏移）
...
2. **开发引导**（`gateway --dev`）
   ...
   - 默认身份：**C3‑PO**（礼仪机器人）。
   - 在开发模式下跳过渠道提供商（`ZOIDBERGBOT_SKIP_CHANNELS=1`）。
...
```bash
ZOIDBERGBOT_PROFILE=dev zoidbergbot gateway --dev --reset
```
...
ZoidbergBot 可以在任何过滤/格式化之前记录**原始助手流**。
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.zoidbergbot/logs/raw-stream.jsonl
```

等效的环境变量：

```bash
ZOIDBERGBOT_RAW_STREAM=1
ZOIDBERGBOT_RAW_STREAM_PATH=~/.zoidbergbot/logs/raw-stream.jsonl
```

默认文件：

`~/.zoidbergbot/logs/raw-stream.jsonl`
