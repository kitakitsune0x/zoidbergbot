---
read_when:
  - 设计超越每日 Markdown 日志的工作区记忆（~/.zoidbergbot/workspace）
  - 决策：独立 CLI 还是深度集成 ZoidbergBot
...
目标：Clawd 风格的工作区（`agents.defaults.workspace`，默认 `~/.zoidbergbot/workspace`），其中"记忆"以每日一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（如 `memory.md`、`SOUL.md`）的形式存储。
...
保持 `~/.zoidbergbot/workspace` 作为规范的人类可读记忆。
...
```
~/.zoidbergbot/workspace/.memory/index.sqlite
```
...
建议：**深度集成到 ZoidbergBot**，但保持核心库可分离。
...
对于 `~/.zoidbergbot/workspace` 的务实建议：
