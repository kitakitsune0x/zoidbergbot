...
read_when:
  - 设置 Mattermost
  - 调试 Mattermost 路由
summary: Mattermost 机器人设置和 ZoidbergBot 配置
title: Mattermost
x-i18n:
...
```bash
zoidbergbot plugins install @zoidbergbot/mattermost
```

本地检出（从 git 仓库运行时）：

```bash
zoidbergbot plugins install ./extensions/mattermost
```

如果你在配置/新手引导期间选择了 Mattermost 并检测到 git 检出，ZoidbergBot 会自动提供本地安装路径。
...
- 通过以下方式批准：
  - `zoidbergbot pairing list mattermost`
  - `zoidbergbot pairing approve mattermost <CODE>`
...
在 `zoidbergbot message send` 或定时任务/webhook 中使用以下目标格式：
