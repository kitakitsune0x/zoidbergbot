...
Matrix 是一个开放、去中心化的消息协议。ZoidbergBot 作为 Matrix **用户**连接到任何主服务器，因此你需要为机器人创建一个 Matrix 账户。登录后，你可以直接私信机器人或邀请它加入房间（Matrix 的"群组"）。Beeper 也是一个可用的客户端选项，但它需要启用端到端加密。
...
```bash
zoidbergbot plugins install @zoidbergbot/matrix
```

本地检出（从 git 仓库运行时）：

```bash
zoidbergbot plugins install ./extensions/matrix
```

如果你在配置/新手引导期间选择了 Matrix 并检测到 git 检出，ZoidbergBot 会自动提供本地安装路径。
...
1. 安装 Matrix 插件：
   - 从 npm：`zoidbergbot plugins install @zoidbergbot/matrix`
   - 从本地检出：`zoidbergbot plugins install ./extensions/matrix`
...
3. 获取机器人账户的访问 token：
   ...
   - 或设置 `channels.matrix.userId` + `channels.matrix.password`：ZoidbergBot 调用相同的登录端点，将访问 token 存储在 `~/.zoidbergbot/credentials/matrix/credentials.json` 中，并在下次启动时重用。
...
- 向加密房间发送时，出站媒体会被加密。
- 首次连接时，ZoidbergBot 会从你的其他会话请求设备验证。
- 在另一个 Matrix 客户端（Element 等）中验证设备以启用密钥共享。
- 如果加密模块无法加载，端到端加密将被禁用且加密房间无法解密；ZoidbergBot 会记录警告。
...
加密状态按账户 + 访问 token 存储在 `~/.zoidbergbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`（SQLite 数据库）。同步状态存储在同一目录下的 `bot-storage.json` 中。如果访问 token（设备）发生变化，会创建新的存储，机器人必须重新验证才能在加密房间中使用。
...
- 通过以下方式批准：
  - `zoidbergbot pairing list matrix`
  - `zoidbergbot pairing approve matrix <CODE>`
...
- 启动时，ZoidbergBot 将允许列表中的房间/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
