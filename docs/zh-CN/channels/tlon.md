...
Tlon 是一个基于 Urbit 构建的去中心化通讯工具。ZoidbergBot 连接到你的 Urbit ship，可以
...
```bash
zoidbergbot plugins install @zoidbergbot/tlon
```

本地签出（从 git 仓库运行时）：

```bash
zoidbergbot plugins install ./extensions/tlon
```
...
与 `zoidbergbot message send` 或定时投递配合使用：
...
- 线程回复：如果收到的消息在线程中，ZoidbergBot 会在线程内回复。
