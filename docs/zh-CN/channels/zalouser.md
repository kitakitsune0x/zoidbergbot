---
read_when:
  - 为 ZoidbergBot 设置 Zalo Personal
...
- 通过 CLI 安装：`zoidbergbot plugins install @zoidbergbot/zalouser`
- 或从源码检出安装：`zoidbergbot plugins install ./extensions/zalouser`
- 详情：[插件](/plugin)
...
2. 登录（二维码方式，在 Gateway网关机器上操作）：
   - `zoidbergbot channels login --channel zalouser`
...
- `zoidbergbot pairing list zalouser`
- `zoidbergbot pairing approve zalouser <code>`
...
- 配置向导可以提示设置群组允许列表。
- 启动时，ZoidbergBot 会将允许列表中的群组/用户名称解析为 ID 并记录映射关系；未解析的条目保持原样。
...
- 重新登录：`zoidbergbot channels logout --channel zalouser && zoidbergbot channels login --channel zalouser`
