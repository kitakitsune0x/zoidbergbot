...
如果你在配置/新手引导期间选择了 Teams 并检测到 git 检出，ZoidbergBot 会自动提供本地安装路径。
...
- 通过 Teams 私信、群聊或频道与 ZoidbergBot 对话。
...
- 启动时，ZoidbergBot 将团队/频道和用户允许列表名称解析为 ID（当 Graph 权限允许时）并记录映射；未解析的条目保持原样。
...
5. 在 `~/.zoidbergbot/zoidbergbot.json`（或环境变量）中配置 `msteams` 并启动 Gateway网关。
6. Gateway网关默认在 `/api/messages` 上监听 Bot Framework webhook 流量。

## Azure Bot 设置（前提条件）

在配置 ZoidbergBot 之前，你需要创建一个 Azure Bot 资源。
...
2. 填写 **Basics** 标签页：
   ...
   | 字段               | 值                                                  |
   | ------------------ | --------------------------------------------------- |
   | **Bot handle**     | 你的机器人名称，例如 `zoidbergbot-msteams`（必须唯一） |
...
1. **安装 Microsoft Teams 插件**
   - 从 npm：`zoidbergbot plugins install @zoidbergbot/msteams`
   - 从本地检出：`zoidbergbot plugins install ./extensions/msteams`
...
ZoidbergBot 通过快速返回并主动发送回复来处理此问题，但非常慢的响应仍可能导致问题。
...
没有 Graph 权限时，包含图片的频道消息将仅作为纯文本接收（机器人无法访问图片内容）。默认情况下，ZoidbergBot 仅从 Microsoft/Teams 主机名下载媒体。通过 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
...
上传的文件存储在已配置 SharePoint 站点默认文档库中的 `/ZoidbergBotShared/` 文件夹。

## 投票（Adaptive Cards）

ZoidbergBot 通过 Adaptive Cards 发送 Teams 投票（没有原生 Teams 投票 API）。

- CLI：`zoidbergbot message poll --channel msteams --target conversation:<id> ...`
- 投票由 Gateway网关记录在 `~/.zoidbergbot/msteams-polls.json` 中。
