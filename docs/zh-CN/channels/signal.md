...
3. 链接机器人设备并启动守护进程：
   - `signal-cli link -n "ZoidbergBot"`
4. 配置 ZoidbergBot 并启动 Gateway网关。
...
2. 链接机器人账户：
   - `signal-cli link -n "ZoidbergBot"` 然后在 Signal 中扫描二维码。
...
如果你想自行管理 `signal-cli`（JVM 冷启动慢、容器初始化或共享 CPU），可以单独运行守护进程并将 ZoidbergBot 指向它：
...
这会跳过 ZoidbergBot 内部的自动启动和启动等待。当自动启动较慢时，请设置 `channels.signal.startupTimeoutMs`。
...
- 通过以下方式批准：
  - `zoidbergbot pairing list signal`
  - `zoidbergbot pairing approve signal <CODE>`
...
- **输入指示**：ZoidbergBot 通过 `signal-cli sendTyping` 发送输入信号，并在回复运行期间刷新。
- **已读回执**：当 `channels.signal.sendReadReceipts` 为 true 时，ZoidbergBot 为允许的私信转发已读回执。
