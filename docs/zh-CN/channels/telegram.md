...
ZoidbergBot 可以使用 `sendMessageDraft` 在 Telegram 私聊中流式传输部分回复。
...
- 如果 Telegram 拒绝 HTML 负载，ZoidbergBot 会以纯文本重试同一条消息。

## 命令（原生 + 自定义）

ZoidbergBot 在启动时将原生命令（如 `/status`、`/reset`、`/model`）注册到 Telegram 的机器人菜单。
你可以通过配置向菜单添加自定义命令：
...
- 自定义命令**仅为菜单条目**；除非你在其他地方处理它们，否则 ZoidbergBot 不会实现它们。
...
**隐私提示：** `@userinfobot` 是第三方机器人。如果你更注重隐私，可以将机器人添加到群组，发送一条消息，然后使用 `zoidbergbot logs --follow` 读取 `chat.id`，或使用 Bot API 的 `getUpdates`。
...
- 群组升级为超级群组时，Telegram 发出 `migrate_to_chat_id`（聊天 ID 变更）。ZoidbergBot 可以自动迁移 `channels.telegram.groups`。
...
Telegram 论坛话题在每条消息中包含 `message_thread_id`。ZoidbergBot：
...
私聊在某些边缘情况下可能包含 `message_thread_id`。ZoidbergBot 保持私聊会话键不变，但在存在 thread id 时仍将其用于回复/草稿流式传输。
...
2. 运行 `zoidbergbot logs --follow` 并查找 `from.id`。
...
ZoidbergBot 支持接收和发送 Telegram 贴纸，并带有智能缓存。

### 接收贴纸

当用户发送贴纸时，ZoidbergBot 根据贴纸类型进行处理：
...
贴纸通过 AI 的视觉能力处理以生成描述。由于相同的贴纸经常被重复发送，ZoidbergBot 会缓存这些描述以避免冗余的 API 调用。
...
**缓存位置：** `~/.zoidbergbot/telegram/sticker-cache.json`
...
ZoidbergBot 使用 Bot API 的 `sendMessageDraft`（非真实消息），然后将
最终回复作为普通消息发送。
...
**表情回应的工作原理：**
Telegram 表情回应作为**独立的 `message_reaction` 事件**到达，而非消息负载中的属性。当用户添加表情回应时，ZoidbergBot：
...
- Telegram 机器人必须在 `allowed_updates` 中显式请求 `message_reaction`（由 ZoidbergBot 自动配置）
...
- 示例：`zoidbergbot message send --channel telegram --target 123456789 --message "hi"`。
...
- `zoidbergbot channels status` 在配置期望接收非提及群组消息时会显示警告。
- `zoidbergbot channels status --probe` 可以额外检查显式数字群组 ID 的成员资格（无法审计通配符 `"*"` 规则）。
...
- 检查 Gateway网关日志：`zoidbergbot logs --follow`（查找 "skipping group message"）
...
- 升级到规范化 abort 信号的 ZoidbergBot 版本，或在 Node 20 上运行 Gateway网关直到可以升级。
