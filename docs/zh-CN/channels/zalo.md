...
- 通过 CLI 安装：`zoidbergbot plugins install @zoidbergbot/zalo`
...
1. 安装 Zalo 插件：
   - 从源码检出安装：`zoidbergbot plugins install ./extensions/zalo`
   - 从 npm 安装（如已发布）：`zoidbergbot plugins install @zoidbergbot/zalo`
   - 或在新手引导中选择 **Zalo** 并确认安装提示
2. 设置令牌：
   - 环境变量：`ZALO_BOT_TOKEN=...`
...
- 批准方式：
  - `zoidbergbot pairing list zalo`
  - `zoidbergbot pairing approve zalo <CODE>`
...
- 使用聊天 ID 作为目标。
- 示例：`zoidbergbot message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**机器人无响应：**

- 检查令牌是否有效：`zoidbergbot channels status --probe`
- 验证发送者是否已批准（配对或 allowFrom）
- 检查 Gateway网关日志：`zoidbergbot logs --follow`
