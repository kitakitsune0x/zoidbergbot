...
通过 IRC 连接支持 Twitch 聊天。ZoidbergBot 以 Twitch 用户（机器人账号）身份连接，在频道中接收和发送消息。
...
4. 配置令牌：
   - 环境变量：`ZOIDBERGBOT_TWITCH_ACCESS_TOKEN=...`（仅限默认账号）
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "zoidbergbot", // 机器人的 Twitch 账号
      accessToken: "oauth:abc123...", // OAuth Access Token（或使用 ZOIDBERGBOT_TWITCH_ACCESS_TOKEN 环境变量）
