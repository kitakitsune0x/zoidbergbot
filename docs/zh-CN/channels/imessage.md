...
3. 配置 ZoidbergBot 的 `channels.imessage.cliPath` 和 `channels.imessage.dbPath`。
...
- macOS 且"信息"已登录。
- ZoidbergBot + `imsg` 需要完全磁盘访问权限（访问 Messages 数据库）。
- 发送时需要自动化权限。
...
如果你想在另一台 Mac 上使用 iMessage，将 `channels.imessage.cliPath` 设置为通过 SSH 在远程 macOS 主机上运行 `imsg` 的包装脚本。ZoidbergBot 只需要 stdio。
...
**远程附件：** 当 `cliPath` 通过 SSH 指向远程主机时，Messages 数据库中的附件路径引用的是远程机器上的文件。ZoidbergBot 可以通过设置 `channels.imessage.remoteHost` 自动通过 SCP 获取这些文件：
...
如果未设置 `remoteHost`，ZoidbergBot 会尝试通过解析你包装脚本中的 SSH 命令来自动检测。建议显式配置以确保可靠性。
...
│ - zoidbergbot gateway           │          SCP（附件）              │ - Messages 已登录         │
...
示例包装脚本（`~/.zoidbergbot/scripts/imsg-ssh`）：
...
多账户支持：使用 `channels.imessage.accounts`，每个账户配置独立选项和可选的 `name`。共享模式请参阅 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。不要提交 `~/.zoidbergbot/zoidbergbot.json`（它通常包含 token）。
...
如果你在 `channels.imessage.groups` 下显式配置了一个 `chat_id`，ZoidbergBot 会将该线程视为"群组"，用于：
