...
3. 创建并下载 **JSON 密钥**：
   ...
   - 选择 **JSON** 并点击 **Create**。
4. 将下载的 JSON 文件存储在你的 Gateway网关主机上（例如 `~/.zoidbergbot/googlechat-service-account.json`）。
5. 在 [Google Cloud Console Chat 配置](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat) 中创建 Google Chat 应用：
   - 填写 **Application info**：
     - **App name**：（例如 `ZoidbergBot`）
     - **Avatar URL**：（例如 `https://zoidbergbot.ai/logo.png`）
   ...
   - 在 **Triggers** 下，选择 **Use a common HTTP endpoint URL for all triggers** 并将其设置为你的 Gateway网关公共 URL 后跟 `/googlechat`。
     - _提示：运行 `zoidbergbot status` 可查找你的 Gateway网关公共 URL。_
...
6. **启用应用状态**：
   ...
   - 再次点击 **Save**。
7. 使用服务账户路径 + webhook audience 配置 ZoidbergBot：
...
Google Chat webhook 需要公共 HTTPS 端点。为安全起见，**仅将 `/googlechat` 路径暴露**到互联网。将 ZoidbergBot 仪表板和其他敏感端点保持在私有网络上。
...
使用此配置，对 `your-domain.com/` 的任何请求将被忽略或返回 404，而 `your-domain.com/googlechat` 安全地路由到 ZoidbergBot。
...
2. ZoidbergBot 根据配置的 `audienceType` + `audience` 验证 token：
...
4. 私信访问默认需要配对。未知发送者会收到配对码；通过以下方式批准：
   - `zoidbergbot pairing approve googlechat <code>`
...
1. **渠道未配置**：配置中缺少 `channels.googlechat` 部分。通过以下方式验证：
   ...
   ```bash
   zoidbergbot config get channels.googlechat
   ```
...
2. **插件未启用**：检查插件状态：
   ...
   ```bash
   zoidbergbot plugins list | grep googlechat
   ```
...
- 检查 `zoidbergbot channels status --probe` 查看认证错误或缺失的 audience 配置。
- 如果没有消息到达，确认 Chat 应用的 webhook URL + 事件订阅。
- 如果提及门控阻止了回复，将 `botUser` 设置为应用的用户资源名称并验证 `requireMention`。
- 发送测试消息时使用 `zoidbergbot logs --follow` 查看请求是否到达 Gateway网关。
