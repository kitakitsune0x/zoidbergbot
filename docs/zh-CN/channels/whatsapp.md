...
2. 在 `~/.zoidbergbot/zoidbergbot.json` 中配置 WhatsApp。
3. 运行 `zoidbergbot channels login` 扫描二维码（已关联设备）。
4. 启动 Gateway网关。
...
WhatsApp 需要真实的手机号码进行验证。VoIP 和虚拟号码通常会被屏蔽。在 WhatsApp 上运行 ZoidbergBot 有两种支持的方式：

### 专用号码（推荐）

为 ZoidbergBot 使用**单独的手机号码**。最佳用户体验，干净的路由，无自聊天问题。理想设置：**备用/旧 Android 手机 + eSIM**。保持 Wi-Fi 和充电连接，通过二维码关联。

**WhatsApp Business：** 你可以在同一设备上使用不同号码的 WhatsApp Business。非常适合将个人 WhatsApp 分开 — 安装 WhatsApp Business 并在其中注册 ZoidbergBot 号码。
...
快速备选方案：在**你自己的号码**上运行 ZoidbergBot。给自己发消息（WhatsApp "给自己发消息"）进行测试，避免打扰联系人。在设置和实验期间，需要在主手机上读取验证码。**必须启用自聊天模式。**
...
当设置了 `messages.responsePrefix` 时，自聊天回复默认使用 `[{identity.name}]`（否则为 `[zoidbergbot]`）。
...
- 早期 ZoidbergBot 版本支持 Twilio 的 WhatsApp Business 集成。
...
- 登录命令：`zoidbergbot channels login`（通过已关联设备扫描二维码）。
- 多账号登录：`zoidbergbot channels login --account <id>`（`<id>` = `accountId`）。
- 默认账号（省略 `--account` 时）：如果存在则为 `default`，否则为第一个已配置的账号 ID（排序后）。
- 凭证存储在 `~/.zoidbergbot/credentials/whatsapp/<accountId>/creds.json`。
- 备份副本位于 `creds.json.bak`（损坏时恢复）。
- 旧版兼容：早期安装将 Baileys 文件直接存储在 `~/.zoidbergbot/credentials/`。
- 注销：`zoidbergbot channels logout`（或 `--account <id>`）删除 WhatsApp 认证状态（但保留共享的 `oauth.json`）。
...
- **私聊策略**：`channels.whatsapp.dmPolicy` 控制私聊访问（默认：`pairing`）。
  - 配对：未知发送者会收到配对码（通过 `zoidbergbot pairing approve whatsapp <code>` 批准；码在 1 小时后过期）。
...
如果你在**个人 WhatsApp 号码**上运行 ZoidbergBot，启用 `channels.whatsapp.selfChatMode`（参见上方示例配置）。
...
**关联 WhatsApp 后，ZoidbergBot 会给随机联系人发消息吗？**
不会。默认私聊策略是**配对**，因此未知发送者只会收到配对码，其消息**不会被处理**。ZoidbergBot 只回复收到的聊天，或你显式触发的发送（智能体/CLI）。

**WhatsApp 上的配对是如何工作的？**
配对是针对未知发送者的私聊门控：

- 新发送者的首条私聊消息会返回一个短码（消息不会被处理）。
- 通过以下命令批准：`zoidbergbot pairing approve whatsapp <code>`（用 `zoidbergbot pairing list whatsapp` 列出）。
- 码在 1 小时后过期；每个渠道的待处理请求上限为 3 个。

**多人可以在同一个 WhatsApp 号码上使用不同的 ZoidbergBot 实例吗？**
...
- 媒体：
  ...
  - 动态 GIF：WhatsApp 期望带 `gifPlayback: true` 的 MP4 以实现内联循环播放。
    - CLI：`zoidbergbot message send --media <mp4> --gif-playback`
...
- 最佳效果：OGG/Opus。ZoidbergBot 将 `audio/ogg` 重写为 `audio/ogg; codecs=opus`。
...
- 日志文件：`/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log`（可配置）。
...
- 修复：在 Gateway网关主机上运行 `zoidbergbot channels login` 并扫描二维码（WhatsApp → 设置 → 已关联设备）。

**已关联但断开连接 / 重连循环**

- 症状：`channels status` 显示 `running, disconnected` 或警告"已关联但断开连接"。
- 修复：`zoidbergbot doctor`（或重启 Gateway网关）。如果问题持续，通过 `channels login` 重新关联并检查 `zoidbergbot logs --follow`。
