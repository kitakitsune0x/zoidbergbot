...
LINE 通过 LINE Messaging API 连接到 ZoidbergBot。插件作为 Gateway网关上的 webhook 接收器运行，使用你的频道访问 token + 频道密钥进行认证。
...
```bash
zoidbergbot plugins install @zoidbergbot/line
```

本地检出（从 git 仓库运行时）：

```bash
zoidbergbot plugins install ./extensions/line
```
...
```bash
zoidbergbot pairing list line
zoidbergbot pairing approve line <CODE>
```
