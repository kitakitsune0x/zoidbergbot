...
- ZoidbergBot 通过其 REST API 与之通信（`GET /api/v1/ping`、`POST /message/text`、`POST /chat/:id/*`）。
...
- **输入指示器**：ZoidbergBot 发送输入开始事件；BlueBubbles 在发送或超时时自动清除输入状态（通过 DELETE 手动停止不可靠）。
...
ZoidbergBot 可能会呈现*短*消息 ID（例如 `1`、`2`）以节省 token。
...
- 直接句柄：`+15555550123`、`user@example.com`
  - 如果直接句柄没有现有的私聊会话，ZoidbergBot 将通过 `POST /api/v1/chat/new` 创建一个。这需要启用 BlueBubbles Private API。
...
- 配对码在一小时后过期；使用 `zoidbergbot pairing list bluebubbles` 和 `zoidbergbot pairing approve bluebubbles <code>`。
...
- ZoidbergBot 会根据 BlueBubbles 服务器的 macOS 版本自动隐藏已知不可用的操作。如果编辑功能在 macOS 26（Tahoe）上仍然显示，请手动通过 `channels.bluebubbles.actions.edit=false` 禁用。
- 查看状态/健康信息：`zoidbergbot status --all` 或 `zoidbergbot status --deep`。
