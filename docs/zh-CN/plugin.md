...
read_when:
  - 添加或修改插件/扩展
  - 记录插件安装或加载规则
summary: ZoidbergBot 插件/扩展：发现、配置与安全
title: 插件
x-i18n:
...
插件就是一个**小型代码模块**，用于为 ZoidbergBot 扩展额外功能（命令、工具和 Gateway网关 RPC）。

大多数情况下，当你需要某个尚未内置到 ZoidbergBot 核心的功能（或希望将可选功能从主安装中分离出来）时，就会使用插件。
...
```bash
zoidbergbot plugins install @zoidbergbot/voice-call
```
...
- 自 2026.1.15 起，Microsoft Teams 仅以插件形式提供；如果使用 Teams，请安装 `@zoidbergbot/msteams`。
- Memory (Core) — 内置记忆搜索插件（默认通过 `plugins.slots.memory` 启用）
- Memory (LanceDB) — 内置长期记忆插件（自动召回/捕获；设置 `plugins.slots.memory = "memory-lancedb"`）
- [Voice Call](/plugins/voice-call) — `@zoidbergbot/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@zoidbergbot/zalouser`
- [Matrix](/channels/matrix) — `@zoidbergbot/matrix`
- [Nostr](/channels/nostr) — `@zoidbergbot/nostr`
- [Zalo](/channels/zalo) — `@zoidbergbot/zalo`
- [Microsoft Teams](/channels/msteams) — `@zoidbergbot/msteams`
...
ZoidbergBot 插件是**TypeScript 模块**，在运行时通过 jiti 加载。**配置验证不会执行插件代码**；而是使用插件清单和 JSON Schema。参见[插件清单](/plugins/manifest)。
...
- `<workspace>/.zoidbergbot/extensions/*.ts`
- `<workspace>/.zoidbergbot/extensions/*/index.ts`

3. 全局扩展

- `~/.zoidbergbot/extensions/*.ts`
- `~/.zoidbergbot/extensions/*/index.ts`

4. 内置扩展（随 ZoidbergBot 一起发布，**默认禁用**）

- `<openclaw>/extensions/*`

内置插件必须通过 `plugins.entries.<id>.enabled` 或 `zoidbergbot plugins enable <id>` 显式启用。已安装的插件默认启用，但可以用同样的方式禁用。

每个插件的根目录中必须包含一个 `zoidbergbot.plugin.json` 文件。如果路径指向一个文件，插件根目录就是该文件所在目录，且必须包含清单文件。

如果多个插件解析到相同的 id，按上述顺序第一个匹配的生效，较低优先级的副本将被忽略。

### 包集合

插件目录可以包含一个带有 `zoidbergbot.extensions` 的 `package.json`：
...
渠道插件可以通过 `zoidbergbot.channel` 公布新手引导元数据，并通过 `zoidbergbot.install` 提供安装提示。这使核心目录保持无数据状态。

示例：

```json
{
  "name": "@zoidbergbot/nextcloud-talk",
  "zoidbergbot": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@zoidbergbot/nextcloud-talk",
...
ZoidbergBot 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放置在以下位置之一：

- `~/.zoidbergbot/mpm/plugins.json`
- `~/.zoidbergbot/mpm/catalog.json`
- `~/.zoidbergbot/plugins/catalog.json`

或者将 `ZOIDBERGBOT_PLUGIN_CATALOG_PATHS`（或 `ZOIDBERGBOT_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（以逗号/分号/`PATH` 分隔）。每个文件应包含 `{ "entries": [ { "name": "@scope/pkg", "zoidbergbot": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出了 `id`，ZoidbergBot 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
- 插件配置使用 `zoidbergbot.plugin.json` 中嵌入的 JSON Schema（`configSchema`）进行验证。
...
ZoidbergBot 在运行时根据已发现的插件增强 `uiHints`：
...
zoidbergbot plugins install <path>                 # 将本地文件/目录复制到 ~/.zoidbergbot/extensions/<id>
zoidbergbot plugins install ./extensions/voice-call # 相对路径也可以
zoidbergbot plugins install ./plugin.tgz           # 从本地 tarball 安装
zoidbergbot plugins install ./plugin.zip           # 从本地 zip 安装
zoidbergbot plugins install -l ./extensions/voice-call # 链接（不复制）用于开发
zoidbergbot plugins install @zoidbergbot/voice-call # 从 npm 安装
...
插件也可以注册自己的顶级命令（示例：`zoidbergbot voicecall`）。
...
```
import { registerPluginHooksFromDir } from "zoidbergbot/plugin-sdk";
...
- 插件管理的钩子在 `zoidbergbot hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `zoidbergbot hooks` 启用/禁用插件管理的钩子；请改为启用/禁用插件。

## 提供商插件（模型认证）

插件可以注册**模型提供商认证**流程，这样用户可以在 ZoidbergBot 内运行 OAuth 或 API 密钥设置（无需外部脚本）。

通过 `api.registerProvider(...)` 注册提供商。每个提供商公开一个或多个认证方法（OAuth、API 密钥、设备代码等）。这些方法驱动：

- `zoidbergbot models auth login --provider <id> [--method <id>]`
...
- 主包：`zoidbergbot`（本仓库）
- 插件：`@zoidbergbot/*` 下的独立 npm 包（示例：`@zoidbergbot/voice-call`）

发布约定：

- 插件 `package.json` 必须包含 `zoidbergbot.extensions`，其中列出一个或多个入口文件。
- 入口文件可以是 `.js` 或 `.ts`（jiti 在运行时加载 TS）。
- `zoidbergbot plugins install <npm-spec>` 使用 `npm pack`，提取到 `~/.zoidbergbot/extensions/<id>/`，并在配置中启用。
...
- CLI：`zoidbergbot voicecall start|status`
- 工具：`voice_call`
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试），并验证 `zoidbergbot.extensions` 指向构建后的入口点（`dist/index.js`）。
