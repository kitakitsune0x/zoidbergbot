...
- 可选前缀：设置 `gateway.controlUi.basePath`（例如 `/zoidbergbot`）
...
如果页面无法加载，请先启动 Gateway网关：`zoidbergbot gateway`。
...
- 配置：查看/编辑 `~/.zoidbergbot/zoidbergbot.json`（`config.get`、`config.set`）
...
（`tailscale-user-login`）进行认证。ZoidbergBot
