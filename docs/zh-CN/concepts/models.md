...
- `agents.defaults.models` 是 ZoidbergBot 可使用的模型白名单/目录（含别名）。
...
如果设置了 `agents.defaults.models`，它将成为 `/model` 和会话覆盖的**白名单**。当用户选择了不在白名单中的模型时，ZoidbergBot 会返回：
...
- 如果省略提供商，ZoidbergBot 会将输入视为别名或**默认提供商**的模型（仅在模型 ID 中不含 `/` 时有效）。
...
zoidbergbot models set <provider/model>
zoidbergbot models set-image <provider/model>

zoidbergbot models aliases list
zoidbergbot models aliases add <alias> <provider/model>
zoidbergbot models aliases remove <alias>

zoidbergbot models fallbacks list
zoidbergbot models fallbacks add <provider/model>
zoidbergbot models fallbacks remove <provider/model>
zoidbergbot models fallbacks clear

zoidbergbot models image-fallbacks list
zoidbergbot models image-fallbacks add <provider/model>
zoidbergbot models image-fallbacks remove <provider/model>
zoidbergbot models image-fallbacks clear
```

`zoidbergbot models`（无子命令）是 `models status` 的快捷方式。
...
`zoidbergbot models scan` 检查 OpenRouter 的**免费模型目录**，并可选择性地探测模型的工具和图像支持情况。
...
`models.providers` 中的自定义提供商会被写入智能体目录下的 `models.json`（默认为 `~/.zoidbergbot/agents/<agentId>/models.json`）。除非 `models.mode` 设置为 `replace`，否则此文件默认会被合并。
