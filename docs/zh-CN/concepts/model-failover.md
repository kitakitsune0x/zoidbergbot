...
read_when:
  ...
  - 更新认证配置文件或模型的故障转移规则
summary: ZoidbergBot 如何轮换认证配置文件并在模型之间进行故障回退
title: 模型故障转移
x-i18n:
...
ZoidbergBot 对 API 密钥和 OAuth 令牌都使用**认证配置文件**。

- 密钥存储在 `~/.zoidbergbot/agents/<agentId>/agent/auth-profiles.json`（旧版路径：`~/.zoidbergbot/agent/auth-profiles.json`）。
- 配置项 `auth.profiles` / `auth.order` **仅包含元数据和路由信息**（不含密钥）。
- 旧版仅导入的 OAuth 文件：`~/.zoidbergbot/credentials/oauth.json`（首次使用时导入到 `auth-profiles.json`）。
...
配置文件存储在 `~/.zoidbergbot/agents/<agentId>/agent/auth-profiles.json` 的 `profiles` 下。

## 轮换顺序

当一个提供商有多个配置文件时，ZoidbergBot 按如下方式选择顺序：

1. **显式配置**：`auth.order[provider]`（如果已设置）。
2. **已配置的配置文件**：按提供商过滤的 `auth.profiles`。
3. **已存储的配置文件**：`auth-profiles.json` 中该提供商的条目。

如果未配置显式顺序，ZoidbergBot 使用轮询顺序：
...
ZoidbergBot 会**在每个会话中固定选定的认证配置文件**，以保持提供商缓存活跃。
...
它们会优先尝试，但 ZoidbergBot 可能在遇到速率限制/超时时轮换到其他配置文件。
用户固定的配置文件会锁定在该配置文件上；如果它失败且配置了模型回退，
ZoidbergBot 会转移到下一个模型，而不是切换配置文件。
...
ZoidbergBot 会将其标记为冷却状态并移至下一个配置文件。
...
计费/额度失败（例如"额度不足"/"信用余额过低"）被视为可故障转移的，但通常不是临时性的。ZoidbergBot 不会使用短冷却期，而是将配置文件标记为**已禁用**（使用更长的退避时间）并轮换到下一个配置文件/提供商。
...
如果某个提供商的所有配置文件都失败，ZoidbergBot 会移至
