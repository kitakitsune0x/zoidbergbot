...
本页汇总了 ZoidbergBot 处理入站消息、会话、队列、流式传输和推理可见性的方式。
...
渠道在重新连接后可能会重新投递相同的消息。ZoidbergBot 维护一个短期缓存，以渠道/账号/对端/会话/消息 ID 为键，避免重复投递触发额外的智能体运行。
