...
- 聊天中的 `/usage cost`：从 ZoidbergBot 会话日志汇总的本地费用摘要。
- CLI：`zoidbergbot status --usage` 打印完整的按提供商分类的详细信息。
- CLI：`zoidbergbot channels list` 在提供商配置旁打印相同的使用量快照（使用 `--no-usage` 跳过）。
