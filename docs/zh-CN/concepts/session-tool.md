...
- 以原始对话记录格式返回消息数组。
- 当传入 `sessionId` 时，ZoidbergBot 会将其解析为对应的会话键（缺失的 ID 会报错）。
...
- 完成后，ZoidbergBot 运行子智能体**通知步骤**并将结果发布到请求方的聊天渠道。
