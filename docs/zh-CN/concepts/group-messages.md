...
在 `~/.zoidbergbot/zoidbergbot.json` 中添加 `groupChat` 块，以便在 WhatsApp 去除文本正文中可见的 `@` 时，显示名称提及仍然有效：
...
- 正则表达式不区分大小写；它们涵盖了 `@zoidbergbot` 这样的显示名称提及，以及带或不带 `+`/空格的原始号码。
...
1. 将你的 WhatsApp 账号（运行 ZoidbergBot 的账号）添加到群组。
2. 发送 `@zoidbergbot …`（或包含号码）。除非设置了 `groupPolicy: "open"`，否则只有白名单中的发送者才能触发。
...
- 手动冒烟测试：
  - 在群组中发送 `@zoidbergbot` 提及，确认回复中引用了发送者名称。
...
- 会话存储条目将以 `agent:<agentId>:whatsapp:group:<jid>` 的形式出现在会话存储中（默认为 `~/.zoidbergbot/agents/<agentId>/sessions/sessions.json`）；缺少条目仅表示该群组尚未触发过运行。
