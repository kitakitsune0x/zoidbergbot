...
ZoidbergBot 运行一个源自 **pi-mono** 的单一嵌入式智能体运行时。

## 工作区（必需）

ZoidbergBot 使用单一智能体工作区目录（`agents.defaults.workspace`）作为智能体工具和上下文的**唯一**工作目录（`cwd`）。

建议：使用 `openclaw setup` 创建 `~/.zoidbergbot/zoidbergbot.json`（如果不存在）并初始化工作区文件。
...
在 `agents.defaults.workspace` 内，ZoidbergBot 期望这些用户可编辑的文件：
...
在新会话的第一轮对话中，ZoidbergBot 会将这些文件的内容直接注入到智能体上下文中。

空白文件会被跳过。大文件会被裁剪和截断并附带标记，以保持提示词精简（阅读完整文件以获取全部内容）。

如果文件缺失，ZoidbergBot 会注入一行"文件缺失"标记（`openclaw setup` 会创建安全的默认模板）。
...
ZoidbergBot 从三个位置加载 Skills（名称冲突时工作区优先）：

- 内置（随安装包附带）
- 托管/本地：`~/.zoidbergbot/skills`
...
ZoidbergBot 复用了 pi-mono 代码库的部分内容（模型/工具），但**会话管理、发现和工具连接由 ZoidbergBot 负责**。
...
- `~/.zoidbergbot/agents/<agentId>/sessions/<SessionId>.jsonl`

会话 ID 是稳定的，由 ZoidbergBot 选择。
...
- 如果省略提供商，ZoidbergBot 会将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
