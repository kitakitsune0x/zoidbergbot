...
- CLI 辅助命令：`openclaw onboard`、`zoidbergbot models list`、`zoidbergbot models set <provider/model>`。

## 内置提供商（pi-ai 目录）

ZoidbergBot 自带 pi‑ai 目录。这些提供商**无需**配置 `models.providers`；只需设置认证并选择模型即可。
...
- CLI：`openclaw onboard --auth-choice token`（粘贴 setup-token）或 `zoidbergbot models auth paste-token --provider anthropic`
...
- CLI：`openclaw onboard --auth-choice openai-codex` 或 `zoidbergbot models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`zoidbergbot plugins enable google-antigravity-auth`
  - 登录：`zoidbergbot models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`zoidbergbot plugins enable google-gemini-cli-auth`
  - 登录：`zoidbergbot models auth login --provider google-gemini-cli --set-default`
  - 注意：你**无需**将客户端 ID 或密钥粘贴到 `zoidbergbot.json` 中。CLI 登录流程会将令牌存储在 Gateway网关主机的认证配置文件中。
