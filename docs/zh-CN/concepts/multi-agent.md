...
- **会话存储**（聊天历史 + 路由状态），位于 `~/.zoidbergbot/agents/<agentId>/sessions`。
...
Skills 通过每个工作区的 `skills/` 文件夹按智能体隔离，共享 Skills 可从 `~/.zoidbergbot/skills` 获取。参阅[Skills：按智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置文件：`~/.zoidbergbot/zoidbergbot.json`（或 `ZOIDBERGBOT_CONFIG_PATH`）
- 状态目录：`~/.zoidbergbot`（或 `ZOIDBERGBOT_STATE_DIR`）
- 工作区：`~/.zoidbergbot/workspace`（或 `~/.zoidbergbot/workspace-<agentId>`）
- 智能体目录：`~/.zoidbergbot/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.zoidbergbot/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你不做任何配置，ZoidbergBot 会运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键格式为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.zoidbergbot/workspace`（设置了 `ZOIDBERGBOT_PROFILE` 时为 `~/.zoidbergbot/workspace-<profile>`）。
- 状态默认为 `~/.zoidbergbot/agents/main/agent`。
...
```bash
zoidbergbot agents list --bindings
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.zoidbergbot/workspace-alex" },
      { id: "mia", workspace: "~/.zoidbergbot/workspace-mia" },
    ],
  },
  bindings: [
...
`~/.zoidbergbot/zoidbergbot.json`（JSON5）：
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.zoidbergbot/workspace-home",
        agentDir: "~/.zoidbergbot/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.zoidbergbot/workspace-work",
        agentDir: "~/.zoidbergbot/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认值：~/.zoidbergbot/credentials/whatsapp/personal
          // authDir: "~/.zoidbergbot/credentials/whatsapp/personal",
        },
        biz: {
          // 可选覆盖。默认值：~/.zoidbergbot/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.zoidbergbot/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.zoidbergbot/workspace-opus",
