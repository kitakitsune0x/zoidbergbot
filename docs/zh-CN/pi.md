...
本文档描述了 ZoidbergBot 如何与 [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 及其兄弟包（`pi-ai`、`pi-agent-core`、`pi-tui`）集成，以驱动其 AI 智能体能力。

## 概述

ZoidbergBot 使用 pi SDK 将 AI 编程智能体嵌入其消息 Gateway网关架构中。ZoidbergBot 不是将 pi 作为子进程启动或使用 RPC 模式，而是直接导入并通过 `createAgentSession()` 实例化 pi 的 `AgentSession`。这种嵌入式方法提供了：
...
| `pi-tui`          | 终端 UI 组件（用于 ZoidbergBot 的本地 TUI 模式）                                              |
...
├── zoidbergbot-tools.ts              # ZoidbergBot 特定工具
...
2. **自定义替换**：ZoidbergBot 用 `exec`/`process` 替换 bash，为沙箱自定义 read/edit/write
3. **ZoidbergBot 工具**：消息、浏览器、画布、会话、定时任务、Gateway网关等
...
这确保了 ZoidbergBot 的策略过滤、沙箱集成和扩展工具集在各提供商之间保持一致。

## 系统提示词构建

系统提示词在 `buildAgentSystemPrompt()`（`system-prompt.ts`）中构建。它组装一个包含以下部分的完整提示词：工具、工具调用风格、安全防护、ZoidbergBot CLI 参考、Skills、文档、工作区、沙箱、消息、回复标签、语音、静默回复、心跳、运行时元数据，以及启用时的记忆和反应，还有可选的上下文文件和额外系统提示词内容。子智能体使用的最小提示词模式会裁剪部分章节。
...
ZoidbergBot 通过 `guardSessionManager()` 封装它以确保工具结果安全。
...
ZoidbergBot 维护一个认证配置文件存储，每个提供商可有多个 API 密钥：
...
ZoidbergBot 加载自定义 pi 扩展以实现特定行为：
...
ZoidbergBot 还有一个直接使用 pi-tui 组件的本地 TUI 模式：
...
| 方面       | Pi CLI                  | ZoidbergBot 嵌入式                                                                                 |
| ---------- | ----------------------- | ----------------------------------------------------------------------------------------------- |
| 调用方式   | `pi` 命令 / RPC         | 通过 `createAgentSession()` 使用 SDK                                                            |
| 工具       | 默认编程工具            | 自定义 ZoidbergBot 工具套件                                                                        |
| 系统提示词 | AGENTS.md + 提示词      | 按渠道/上下文动态生成                                                                           |
| 会话存储   | `~/.pi/agent/sessions/` | `~/.zoidbergbot/agents/<agentId>/sessions/`（或 `$ZOIDBERGBOT_STATE_DIR/agents/<agentId>/sessions/`） |
| 认证       | 单一凭据                | 多配置文件轮换                                                                                  |
| 扩展       | 从磁盘加载              | 编程式 + 磁盘路径                                                                               |
