---
read_when:
  - 你想在 ZoidbergBot 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指导
summary: 在 ZoidbergBot 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
zoidbergbot plugins enable minimax-portal-auth  # 如已加载则跳过
zoidbergbot gateway restart  # 如 Gateway网关已在运行则重启
...
详情请参阅 [MiniMax OAuth 插件 README](https://github.com/zoidbergbot/zoidbergbot/tree/main/extensions/minimax-portal-auth)。
...
## 通过 `zoidbergbot configure` 配置

使用交互式配置向导设置 MiniMax，无需编辑 JSON：

1. 运行 `zoidbergbot configure`。
2. 选择 **Model/auth**。
3. 选择 **MiniMax M2.1**。
4. 在提示时选择你的默认模型。
...
- 使用 `zoidbergbot models list` 和 `zoidbergbot models set minimax/MiniMax-M2.1` 进行切换。
...
- 运行 `zoidbergbot configure` 并选择 **MiniMax M2.1**，或
