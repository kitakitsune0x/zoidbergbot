---
read_when:
  - 你想在 ZoidbergBot 中使用 Qwen
  - 你想通过免费版 OAuth 访问 Qwen Coder
summary: 在 ZoidbergBot 中使用 Qwen OAuth（免费版）
title: Qwen
x-i18n:
...
```bash
zoidbergbot models set qwen-portal/coder-model
```

## 复用 Qwen Code CLI 登录

如果你已经通过 Qwen Code CLI 登录，ZoidbergBot 在加载认证存储时会从 `~/.qwen/oauth_creds.json` 同步凭据。你仍然需要一个 `models.providers.qwen-portal` 条目（使用上面的登录命令创建）。
