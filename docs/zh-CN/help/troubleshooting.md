...
你也可以设置 `ZOIDBERGBOT_VERBOSE=1` 来代替该标志。
...
### `docs.zoidbergbot.ai` 显示 SSL 错误（Comcast/Xfinity）

某些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 屏蔽 `docs.zoidbergbot.ai`。
禁用 Advanced Security 或将 `docs.zoidbergbot.ai` 添加到允许列表，然后重试。
...
- 检查允许列表：`zoidbergbot config get agents.defaults.models`
...
如果可以，请附上 `zoidbergbot logs --follow` 中相关的日志尾部内容。
