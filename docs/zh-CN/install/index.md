...
read_when:
  - 安装 ZoidbergBot
  - 你想从 GitHub 安装
summary: 安装 ZoidbergBot（推荐安装器、全局安装或从源码安装）
title: 安装
x-i18n:
...
通过 npm 全局安装 `zoidbergbot` 并运行新手引导。
...
pnpm add -g zoidbergbot@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果尚未全局安装，可通过 `pnpm zoidbergbot ...` 运行仓库命令。
...
- 快速检查：`zoidbergbot doctor`
- 检查 Gateway网关健康状态：`zoidbergbot status` + `openclaw health`
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不在** `echo "$PATH"` 的输出中，说明你的 shell 无法找到全局 npm 二进制文件（包括 `zoidbergbot`）。
