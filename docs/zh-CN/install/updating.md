...
read_when:
  - 更新 ZoidbergBot
  - 更新后出现问题
summary: 安全更新 ZoidbergBot（全局安装或源码安装），以及回滚策略
title: 更新
x-i18n:
...
ZoidbergBot 迭代速度很快（尚未到"1.0"）。请像对待基础设施发布一样对待更新：更新 → 运行检查 → 重启（或使用 `zoidbergbot update`，它会自动重启）→ 验证。

## 推荐方式：重新运行网站安装程序（原地升级）

**首选**更新路径是重新运行网站上的安装程序。它会检测现有安装、原地升级，并在需要时运行 `zoidbergbot doctor`。
...
- 对于**全局安装**，脚本底层使用 `npm install -g zoidbergbot@latest`。
- 兼容性说明：`zoidbergbot` 仍可作为兼容性垫片使用。
...
- 备份你的自定义配置：
  - 配置文件：`~/.zoidbergbot/zoidbergbot.json`
  - 凭据：`~/.zoidbergbot/credentials/`
  - 工作区：`~/.zoidbergbot/workspace`
...
- 如果你的 Gateway网关作为服务运行，建议使用 `zoidbergbot gateway restart` 而非直接终止 PID。
- 如果你固定在特定版本，请参阅下方的"回滚/版本固定"。

## 更新（`zoidbergbot update`）
...
- 切换到所选渠道（标签或分支）。
- 从配置的上游（dev 渠道）拉取并变基。
- 安装依赖、构建、构建控制面板 UI，并运行 `zoidbergbot doctor`。
- 默认重启 Gateway网关（使用 `--no-restart` 跳过）。

如果你通过 **npm/pnpm** 安装（无 git 元数据），`zoidbergbot update` 会尝试通过你的包管理器进行更新。如果无法检测到安装方式，请改用"更新（全局安装）"。

## 更新（控制面板 UI / RPC）

控制面板 UI 提供**更新并重启**功能（RPC：`update.run`）。它会：

1. 执行与 `zoidbergbot update` 相同的源码更新流程（仅限 git checkout）。
...
- 当你运行打包后的 `zoidbergbot` 二进制文件（[`zoidbergbot.mjs`](https://github.com/zoidbergbot/zoidbergbot/blob/main/zoidbergbot.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
- 如果你从仓库检出运行而没有全局安装，请使用 `pnpm zoidbergbot ...` 执行 CLI 命令。
- 如果你直接从 TypeScript 运行（`pnpm zoidbergbot ...`），通常不需要重新构建，但**配置迁移仍然适用** → 运行 doctor。
- 在全局安装和 git 安装之间切换很容易：安装另一种方式，然后运行 `zoidbergbot doctor`，这样 Gateway网关服务入口点会被重写为当前安装。

## 必须执行：`zoidbergbot doctor`

Doctor 是"安全更新"命令。它有意设计得很朴素：修复 + 迁移 + 警告。

注意：如果你使用的是**源码安装**（git checkout），`zoidbergbot doctor` 会建议先运行 `zoidbergbot update`。
...
- 检测并将旧版 Gateway网关服务（launchd/systemd；旧版 schtasks）迁移到当前的 ZoidbergBot 服务。
...
- macOS launchd（应用捆绑的 LaunchAgent）：`launchctl kickstart -k gui/$UID/bot.molt.gateway`（使用 `bot.molt.<profile>`；旧版 `com.zoidbergbot.*` 仍可用）
- Linux systemd 用户服务：`systemctl --user restart zoidbergbot-gateway[-<profile>].service`
- Windows（WSL2）：`systemctl --user restart zoidbergbot-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则请运行 `zoidbergbot gateway install`。
...
提示：要查看当前已发布的版本，请运行 `npm view zoidbergbot version`。
...
- 再次运行 `zoidbergbot doctor` 并仔细阅读输出（它通常会告诉你修复方法）。
