...
当你**显式**使用 `--channel` 切换渠道时，ZoidbergBot 也会同步调整安装方式：

- `dev` 确保存在 git checkout（默认 `~/zoidbergbot`，可通过 `ZOIDBERGBOT_GIT_DIR` 覆盖），更新它，并从该 checkout 安装全局 CLI。
...
使用 `zoidbergbot update` 切换渠道时，ZoidbergBot 也会同步插件来源：
