---
read_when:
  - 你正在将 ZoidbergBot 迁移到新的笔记本/服务器
  - 你想保留会话、认证和渠道登录状态（WhatsApp 等）
summary: 将 ZoidbergBot 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
# 将 ZoidbergBot 迁移到新机器

本指南介绍如何将 ZoidbergBot Gateway网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$ZOIDBERGBOT_STATE_DIR`，默认：`~/.zoidbergbot/`）— 包含配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.zoidbergbot/workspace/`）— 包含你的智能体文件（记忆、提示词等）。
...
- `--profile <name>`（通常变为 `~/.zoidbergbot-<profile>/`）
...
在输出中查找 `ZOIDBERGBOT_STATE_DIR` / profile 的相关信息。如果你运行了多个 Gateway网关，请对每个配置文件重复操作。

### 2) 确认你的工作区

常见默认路径：

- `~/.zoidbergbot/workspace/`（推荐工作区）
- 你创建的自定义文件夹
...
### 步骤 1 — 在新机器上安装 ZoidbergBot

在**新**机器上安装 CLI（如有需要也安装 Node）：

- 参见：[安装](/install)

在此阶段，新手引导创建一个全新的 `~/.zoidbergbot/` 是没问题的 — 你将在下一步覆盖它。

### 步骤 2 — 将状态目录 + 工作区复制到新机器

**同时**复制：

- `$ZOIDBERGBOT_STATE_DIR`（默认 `~/.zoidbergbot/`）
- 你的工作区（默认 `~/.zoidbergbot/workspace/`）
...
如果旧 Gateway网关使用了配置文件（或 `ZOIDBERGBOT_STATE_DIR`），而新 Gateway网关使用了不同的路径，你会看到以下症状：
...
### 坑：只复制了 `zoidbergbot.json`

`zoidbergbot.json` 是不够的。许多提供商将状态存储在：

- `$ZOIDBERGBOT_STATE_DIR/credentials/`
- `$ZOIDBERGBOT_STATE_DIR/agents/<agentId>/...`

始终迁移整个 `$ZOIDBERGBOT_STATE_DIR` 文件夹。
...
`$ZOIDBERGBOT_STATE_DIR` 包含密钥（API 密钥、OAuth 令牌、WhatsApp 凭据）。请将备份视为生产密钥：
...
- `zoidbergbot status` 显示 Gateway网关正在运行
