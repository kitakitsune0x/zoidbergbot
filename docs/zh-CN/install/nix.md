...
read_when:
  ...
  - 你想要一切固定且声明式管理
summary: 使用 Nix 声明式安装 ZoidbergBot
title: Nix
x-i18n:
...
ZoidbergBot 从 `ZOIDBERGBOT_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `ZOIDBERGBOT_STATE_DIR` 中。

- `ZOIDBERGBOT_STATE_DIR`（默认：`~/.zoidbergbot`）
- `ZOIDBERGBOT_CONFIG_PATH`（默认：`$ZOIDBERGBOT_STATE_DIR/zoidbergbot.json`）
...
[`scripts/package-mac-app.sh`](https://github.com/zoidbergbot/zoidbergbot/blob/main/scripts/package-mac-app.sh) 将此模板复制到应用包中并修补动态字段
