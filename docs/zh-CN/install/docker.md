...
read_when:
  ...
  - 你正在验证 Docker 流程
summary: 可选的基于 Docker 的 ZoidbergBot 设置和新手引导
title: Docker
x-i18n:
...
- `ZOIDBERGBOT_DOCKER_APT_PACKAGES` — 在构建期间安装额外的 apt 软件包
- `ZOIDBERGBOT_EXTRA_MOUNTS` — 添加额外的主机绑定挂载
- `ZOIDBERGBOT_HOME_VOLUME` — 将 `/home/node` 持久化到命名卷
...
如果你想将额外的主机目录挂载到容器中，请在运行 `docker-setup.sh` 之前设置 `ZOIDBERGBOT_EXTRA_MOUNTS`。该变量接受逗号分隔的 Docker 绑定挂载列表，并通过生成 `docker-compose.extra.yml` 将其应用到 `zoidbergbot-gateway` 和 `zoidbergbot-cli`。
...
- 如果修改了 `ZOIDBERGBOT_EXTRA_MOUNTS`，请重新运行 `docker-setup.sh` 以重新生成额外的 compose 文件。
- `docker-compose.extra.yml` 是自动生成的。请勿手动编辑。

### 持久化整个容器 home 目录（可选）

如果你希望 `/home/node` 在容器重建后持久保留，请通过 `ZOIDBERGBOT_HOME_VOLUME` 设置命名卷。这会创建一个 Docker 卷并挂载到 `/home/node`，同时保留标准的配置/工作区绑定挂载。此处请使用命名卷（而非绑定路径）；绑定挂载请使用 `ZOIDBERGBOT_EXTRA_MOUNTS`。
...
- 如果修改了 `ZOIDBERGBOT_HOME_VOLUME`，请重新运行 `docker-setup.sh` 以重新生成额外的 compose 文件。
- 命名卷会一直保留，直到通过 `docker volume rm <name>` 删除。

### 安装额外的 apt 软件包（可选）

如果你需要在镜像中安装系统软件包（例如构建工具或媒体库），请在运行 `docker-setup.sh` 之前设置 `ZOIDBERGBOT_DOCKER_APT_PACKAGES`。这会在镜像构建期间安装软件包，因此即使容器被删除也会保留。
...
- 如果修改了 `ZOIDBERGBOT_DOCKER_APT_PACKAGES`，请重新运行 `docker-setup.sh` 以重建镜像。
...
- Gateway网关容器是会话的权威来源（`~/.zoidbergbot/agents/<agentId>/sessions/`）。
...
- 镜像：`zoidbergbot-sandbox:bookworm-slim`
- 每个智能体一个容器
- 智能体工作区访问：`workspaceAccess: "none"`（默认）使用 `~/.zoidbergbot/sandboxes`
...
- `user` 必须是 root 才能运行 `apt-get`（省略 `user` 或设置 `user: "0:0"`）。
  当 `setupCommand`（或 docker 配置）发生变更时，ZoidbergBot 会自动重建容器，除非容器**最近被使用过**（约 5 分钟内）。活跃容器会记录一条警告，包含确切的 `zoidbergbot sandbox recreate ...` 命令。
...
这会使用 `Dockerfile.sandbox` 构建 `zoidbergbot-sandbox:bookworm-slim`。
...
这会构建 `zoidbergbot-sandbox-common:bookworm-slim`。使用方法：
...
这会使用 `Dockerfile.sandbox-browser` 构建 `zoidbergbot-sandbox-browser:bookworm-slim`。容器运行启用了 CDP 的 Chromium，并提供可选的 noVNC 观察器（通过 Xvfb 实现有头模式）。
...
- 镜像缺失：使用 [`scripts/sandbox-setup.sh`](https://github.com/zoidbergbot/zoidbergbot/blob/main/scripts/sandbox-setup.sh) 构建或设置 `agents.defaults.sandbox.docker.image`。
- 容器未运行：它会按需在每个会话中自动创建。
- 沙箱中的权限错误：将 `docker.user` 设置为与挂载工作区所有权匹配的 UID:GID（或对工作区文件夹执行 chown）。
- 找不到自定义工具：ZoidbergBot 使用 `sh -lc`（登录 shell）运行命令，这会加载 `/etc/profile` 并可能重置 PATH。请设置 `docker.env.PATH` 以前置你的自定义工具路径（例如 `/custom/bin:/usr/local/share/npm-global/bin`），或在 Dockerfile 中的 `/etc/profile.d/` 下添加脚本。
