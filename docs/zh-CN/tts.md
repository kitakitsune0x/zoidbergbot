...
ZoidbergBot 可以使用 ElevenLabs、OpenAI 或 Edge TTS 将出站回复转换为音频。
它适用于 ZoidbergBot 能发送音频的任何地方；Telegram 会显示圆形语音消息气泡。
...
Edge TTS **不**需要 API 密钥。如果未找到任何 API 密钥，ZoidbergBot 默认使用 Edge TTS（除非通过 `messages.tts.edge.enabled=false` 禁用）。
...
TTS 配置位于 `zoidbergbot.json` 中的 `messages.tts` 下。
...
- 如果 `provider` **未设置**，ZoidbergBot 优先使用 `openai`（如有密钥），然后 `elevenlabs`（如有密钥），否则使用 `edge`。
...
斜杠命令将本地覆盖写入 `prefsPath`（默认：`~/.zoidbergbot/settings/tts.json`，可通过 `ZOIDBERGBOT_TTS_PREFS` 或 `messages.tts.prefsPath` 覆盖）。
...
- **Edge TTS**：使用 `edge.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`）。
  ...
  - 如果配置的 Edge 输出格式失败，ZoidbergBot 会以 MP3 重试。
...
Discord 说明：`/tts` 是 Discord 的内置命令，因此 ZoidbergBot 在该平台注册 `/voice` 作为原生命令。文本命令 `/tts ...` 仍然有效。
...
/tts limit 2000
/tts summary off
/tts audio Hello from ZoidbergBot
```
