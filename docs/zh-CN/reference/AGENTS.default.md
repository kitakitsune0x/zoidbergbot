---
read_when:
  - 开始新的 ZoidbergBot 智能体会话
  - 启用或审查默认 Skills
summary: 默认 ZoidbergBot 智能体指令及个人助手设置的 Skills 清单
x-i18n:
...
# AGENTS.md — ZoidbergBot 个人助手（默认）

## 首次运行（推荐）

ZoidbergBot 为智能体使用专用的工作区目录。默认路径：`~/.zoidbergbot/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
```bash
cp docs/reference/templates/AGENTS.md ~/.zoidbergbot/workspace/AGENTS.md
cp docs/reference/templates/SOUL.md ~/.zoidbergbot/workspace/SOUL.md
cp docs/reference/templates/TOOLS.md ~/.zoidbergbot/workspace/TOOLS.md
```

3. 可选：如果你需要个人助手 Skills 清单，请用此文件替换 AGENTS.md：

```bash
cp docs/reference/AGENTS.default.md ~/.zoidbergbot/workspace/AGENTS.md
```
...
- macOS 应用管理权限（屏幕录制、通知、麦克风），并通过其内置二进制文件提供 `zoidbergbot` CLI。
...
- 脚本编写优先使用 `zoidbergbot` CLI；mac 应用负责处理权限。
...
- 对于浏览器驱动的验证，请使用 `zoidbergbot browser`（tabs/status/screenshot），配合 ZoidbergBot 管理的 Chrome 配置文件。
- 对于 DOM 检查，请使用 `zoidbergbot browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互操作，请使用 `zoidbergbot browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要快照引用；使用 `evaluate` 进行 CSS 选择器操作）。
