...
ZoidbergBot 围绕单个 **Gateway网关进程**设计，该进程拥有会话状态。
...
- 存储：`~/.zoidbergbot/agents/<agentId>/sessions/sessions.json`
- 对话记录：`~/.zoidbergbot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram 话题会话：`.../<sessionId>-topic-<threadId>.jsonl`

ZoidbergBot 通过 `src/config/sessions.ts` 解析这些路径。
...
ZoidbergBot 有意**不**"修复"对话记录；Gateway网关使用 `SessionManager` 来读写它们。
...
这些是 Pi 运行时语义（ZoidbergBot 消费事件，但由 Pi 决定何时压缩）。
...
ZoidbergBot 还为嵌入式运行强制执行安全下限：

- 如果 `compaction.reserveTokens < reserveTokensFloor`，ZoidbergBot 会将其提升。
- 默认下限为 `20000` token。
- 设置 `agents.defaults.compaction.reserveTokensFloor: 0` 可禁用下限。
- 如果已经更高，ZoidbergBot 不做修改。
...
ZoidbergBot 支持用于后台任务的"静默"轮次，用户不应看到中间输出。

约定：

- 助手以 `NO_REPLY` 开始其输出，表示"不要向用户传递回复"。
- ZoidbergBot 在传递层中剥离/抑制此标记。

自 `2026.1.10` 起，当部分块以 `NO_REPLY` 开头时，ZoidbergBot 还会抑制**草稿/输入中的流式传输**，因此静默操作不会在轮次中途泄露部分输出。
...
Pi 也在扩展 API 中暴露了 `session_before_compact` 钩子，但 ZoidbergBot 的刷写逻辑目前在 Gateway网关侧。
...
- 存储与对话记录不匹配？确认 Gateway网关主机以及 `zoidbergbot status` 中的存储路径。
- 压缩过于频繁？检查：
  - 模型上下文窗口（太小）
