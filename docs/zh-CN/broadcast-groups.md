...
广播群组在渠道允许列表和群组激活规则之后进行评估。在 WhatsApp 群组中，这意味着广播发生在 ZoidbergBot 正常回复的时机（例如：被提及时，取决于你的群组设置）。
...
**效果：** 当 ZoidbergBot 在此聊天中回复时，它会运行所有三个智能体。
...
工作区：/Users/pascal/zoidbergbot-alfred/
工具：read、write、exec
```
...
工作区：/Users/pascal/zoidbergbot-baerbel/
工具：仅 read
```
...
```bash
tail -f ~/.zoidbergbot/logs/gateway.log | grep broadcast
```
