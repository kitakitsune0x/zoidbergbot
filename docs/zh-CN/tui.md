...
```bash
zoidbergbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- 开启投递：
  - `/deliver on`
  - 或通过设置面板
  - 或启动时使用 `zoidbergbot tui --deliver`
...
- 确认智能体可以运行：`zoidbergbot status` 和 `zoidbergbot models status`。
...
- 选择器中没有智能体：检查 `zoidbergbot agents list` 和你的路由配置。
