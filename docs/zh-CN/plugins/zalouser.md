---
read_when:
  - 你想在 ZoidbergBot 中使用 Zalo Personal（非官方）支持
...
通过插件为 ZoidbergBot 提供 Zalo Personal 支持，使用 `zca-cli` 自动化操作普通 Zalo 用户账号。
...
```bash
zoidbergbot plugins install @zoidbergbot/zalouser
```
...
```bash
zoidbergbot plugins install ./extensions/zalouser
...
zoidbergbot channels logout --channel zalouser
zoidbergbot channels status --probe
zoidbergbot message send --channel zalouser --target <threadId> --message "Hello from ZoidbergBot"
