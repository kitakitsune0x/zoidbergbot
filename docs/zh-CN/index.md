...
read_when:
  - 向新用户介绍 ZoidbergBot
summary: ZoidbergBot 的顶层概述、功能特性与用途
title: ZoidbergBot
x-i18n:
...
> **中文文档提示：** 本页及其他中文文档由自动化翻译流水线生成。如果你发现翻译问题，请在 [#6995](https://github.com/zoidbergbot/zoidbergbot/issues/6995) 反馈（不要提交 PR）。我们正在积极扩展对中文用户、模型与消息平台的支持，更多内容即将推出！需要支持请到 Discord 的 [#help-中文](https://discord.com/channels/1456350064065904867/1466722439789674741)。
...
<p align="center">
  ...
  <a href="https://github.com/zoidbergbot/zoidbergbot/releases">版本发布</a> ·
  <a href="/">文档</a> ·
  <a href="/start/zoidbergbot">ZoidbergBot 助手设置</a>
</p>

ZoidbergBot 将 WhatsApp（通过 WhatsApp Web / Baileys）、Telegram（Bot API / grammY）、Discord（Bot API / channels.discord.js）和 iMessage（imsg CLI）桥接至编程智能体，例如 [Pi](https://github.com/badlogic/pi-mono)。插件可添加 Mattermost（Bot API + WebSocket）等更多渠道支持。
ZoidbergBot 同时也驱动着 ZoidbergBot 助手。
...
如果 Gateway网关运行在这台计算机上，该链接会立即打开浏览器控制界面。如果无法打开，请先启动 Gateway网关： `zoidbergbot gateway`.
...
大多数操作通过 **Gateway网关** （`zoidbergbot gateway`进行，它是一个长期运行的单进程，负责管理渠道连接和 WebSocket 控制面。
...
- **优先 local loopback**：Gateway网关 WS 默认监听 `ws://127.0.0.1:18789`.
  ...
  - 如需 Tailnet 访问，请运行 `zoidbergbot gateway --bind tailnet --token ...` （非 local loopback 绑定时必须提供令牌）。
- **节点**：通过 WebSocket 连接到 Gateway网关（根据需要使用局域网/Tailnet/SSH）；旧版 TCP 桥接已弃用/移除。
- **Canvas 主机**：HTTP 文件服务器运行在 `canvasHost.port` （默认 `18793`），提供 `/__zoidbergbot__/canvas/` 用于节点 WebView；请参阅 [Gateway网关配置](/gateway/configuration) （`canvasHost`）。
...
之后在 npm 安装和 git 安装之间切换很简单：安装另一种方式并运行 `zoidbergbot doctor` 以更新 Gateway网关服务入口点。
...
如果尚未进行全局安装，请通过以下方式运行新手引导流程 `pnpm zoidbergbot ...` （在仓库目录中执行）。
...
配置文件位于 `~/.zoidbergbot/zoidbergbot.json`.

- 如果你 **不做任何操作**，ZoidbergBot 将使用内置的 Pi 二进制文件以 RPC 模式运行，并采用按发送者区分的会话。
...
- 从这里开始：
  ...
  - [Nix 模式](/install/nix)
  - [ZoidbergBot 助手设置](/start/zoidbergbot)
  - [Skills](/tools/skills)
...
**ZoidbergBot = CLAW + TARDIS** — 因为每只太空龙虾都需要一台时空机器。
