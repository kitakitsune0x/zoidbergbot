---
read_when:
  - 在 Oracle Cloud 上部署 ZoidbergBot
  - 寻找低成本 VPS 托管方案来运行 ZoidbergBot
  - 希望在小型服务器上全天候运行 ZoidbergBot
summary: 在 Oracle Cloud（Always Free ARM）上运行 ZoidbergBot
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud (OCI) 上运行 ZoidbergBot

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层级上运行持久化的 ZoidbergBot Gateway网关。

Oracle 的免费层级非常适合运行 ZoidbergBot（特别是如果你已有 OCI 账户），但也存在一些权衡：
...
这种配置通常不再*需要*额外的主机防火墙规则来阻止全网 SSH 暴力破解——但你仍应保持操作系统更新、运行 `zoidbergbot security audit`，并确认没有意外监听公共接口。
...
- **凭据权限：** `chmod 700 ~/.zoidbergbot`
- **安全审计：** `zoidbergbot security audit`
...
- `~/.zoidbergbot/` — 配置、凭据、会话数据
- `~/.zoidbergbot/workspace/` — 工作区（SOUL.md、记忆、产物）
