...
推荐**通过 WSL2** 在 Windows 上使用 ZoidbergBot（建议使用 Ubuntu）。CLI + Gateway网关在 Linux 内运行，这样可以保持运行时一致性，并使工具链兼容性更好（Node/Bun/pnpm、Linux 二进制文件、Skills）。原生 Windows 可能会更麻烦。WSL2 为你提供完整的 Linux 体验——一条命令即可安装：`wsl --install`。
...
- 远程节点必须指向一个**可达的** Gateway网关 URL（而非 `127.0.0.1`）；使用 `zoidbergbot status --all` 来确认。
