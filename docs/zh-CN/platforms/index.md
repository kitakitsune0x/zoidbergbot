...
ZoidbergBot 核心使用 TypeScript 编写。**推荐使用 Node 作为运行时**。
...
- 服务状态：`zoidbergbot gateway status`
...
- 直接安装：`zoidbergbot gateway install`
- 配置流程：`zoidbergbot configure` → 选择 **Gateway网关 service**
- 修复/迁移：`zoidbergbot doctor`（会提示安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.zoidbergbot.*`）
- Linux/WSL2：systemd 用户服务（`zoidbergbot-gateway[-<profile>].service`）
