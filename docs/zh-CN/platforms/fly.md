...
**目标：** 在 [Fly.io](https://fly.io) 机器上运行 ZoidbergBot Gateway网关，配备持久化存储、自动 HTTPS 和 Discord/渠道访问。
...
| `internal_port = 3000`         | 必须与 `--port 3000`（或 `ZOIDBERGBOT_GATEWAY_PORT`）匹配，用于 Fly 健康检查 |
| `memory = "2048mb"`            | 512MB 太小；推荐 2GB                                                      |
| `ZOIDBERGBOT_STATE_DIR = "/data"` | 将状态持久化到卷上                                                        |
...
- 非 local loopback 绑定（`--bind lan`）出于安全需要 `ZOIDBERGBOT_GATEWAY_TOKEN`。
- 请像对待密码一样对待这些令牌。
- **所有 API 密钥和令牌优先使用环境变量而非配置文件**。这样可以避免密钥出现在 `zoidbergbot.json` 中，防止意外暴露或被记录到日志。
...
**注意：** 当 `ZOIDBERGBOT_STATE_DIR=/data` 时，配置路径为 `/data/zoidbergbot.json`。
...
或访问 `https://my-zoidbergbot.fly.dev/`

粘贴你的 Gateway网关令牌（即 `ZOIDBERGBOT_GATEWAY_TOKEN` 的值）进行认证。
...
**修复：** 确保 `internal_port` 与 Gateway网关端口匹配（设置 `--port 3000` 或 `ZOIDBERGBOT_GATEWAY_PORT=3000`）。
...
如果使用 `--allow-unconfigured`，Gateway网关会创建一个最小配置。你在 `/data/zoidbergbot.json` 的自定义配置应在重启后被读取。
...
**修复：** 确保 `fly.toml` 中设置了 `ZOIDBERGBOT_STATE_DIR=/data` 并重新部署。
