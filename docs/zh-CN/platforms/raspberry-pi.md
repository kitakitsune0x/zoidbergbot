---
read_when:
  - 在 Raspberry Pi 上设置 ZoidbergBot
  - 在 ARM 设备上运行 ZoidbergBot
  - 搭建低成本的全天候个人 AI
summary: 在 Raspberry Pi 上运行 ZoidbergBot（低成本自托管方案）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 ZoidbergBot

## 目标

在 Raspberry Pi 上运行一个持久的、全天候在线的 ZoidbergBot Gateway网关，**一次性费用约 $35-80**（无月费）。
...
大多数 ZoidbergBot 功能在 ARM64 上正常工作，但部分外部二进制文件可能需要 ARM 构建版本：
...
```bash
# 检查日志
journalctl -u zoidbergbot --no-pager -n 100

# 常见修复方法：重新构建
cd ~/zoidbergbot  # 如果使用可修改安装
npm run build
sudo systemctl restart openclaw
```
