---
read_when:
  - 你希望在 GCP 上全天候运行 ZoidbergBot
  - 你希望在自己的虚拟机上运行生产级、始终在线的 Gateway网关
  - 你希望完全掌控持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine 虚拟机上全天候运行 ZoidbergBot Gateway网关（Docker），并实现持久化状态存储
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 ZoidbergBot（Docker，生产环境 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine 虚拟机上运行持久化的 ZoidbergBot Gateway网关，实现持久状态存储、内置二进制文件和安全的重启行为。

如果你想"以每月约 $5-12 的成本全天候运行 ZoidbergBot"，这是在 Google Cloud 上的可靠方案。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 ZoidbergBot Gateway网关
- 将 `~/.zoidbergbot` + `~/.zoidbergbot/workspace` 持久化到宿主机（重启/重建后数据不丢失）
...
ZoidbergBot 在 Docker 中运行，但 Docker 并非数据来源。
所有长期状态必须在重启、重建和重启动后仍然存在。

| 组件            | 位置                              | 持久化机制      | 备注                        |
| --------------- | --------------------------------- | --------------- | --------------------------- |
| Gateway网关配置 | `/home/node/.zoidbergbot/`           | 宿主机卷挂载    | 包含 `zoidbergbot.json`、令牌  |
| 模型认证配置    | `/home/node/.zoidbergbot/`           | 宿主机卷挂载    | OAuth 令牌、API 密钥        |
| Skills配置      | `/home/node/.zoidbergbot/skills/`    | 宿主机卷挂载    | Skills 级别状态             |
| 智能体工作区    | `/home/node/.zoidbergbot/workspace/` | 宿主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话   | `/home/node/.zoidbergbot/`           | 宿主机卷挂载    | 保留二维码登录              |
| Gmail 密钥环    | `/home/node/.zoidbergbot/`           | 宿主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
