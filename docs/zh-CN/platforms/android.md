...
- 你可以在 Gateway网关机器上（或通过 SSH）运行 CLI（`zoidbergbot`）。
...
- 在 Gateway网关主机的 `~/.zoidbergbot/zoidbergbot.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway网关主机上设置 DNS-SD 区域（示例 `zoidbergbot.internal.`）并发布 `_zoidbergbot-gw._tcp` 记录。
...
1. 在 Gateway网关主机上创建 `~/.zoidbergbot/workspace/canvas/index.html`。

2. 将节点导航到该地址（局域网）：

```bash
zoidbergbot nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__zoidbergbot__/canvas/"}'
```

Tailnet（可选）：如果两台设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 替代 `.local`，例如 `http://<gateway-magicdns>:18793/__zoidbergbot__/canvas/`。

该服务器会向 HTML 注入实时重载客户端，并在文件变更时重新加载。
A2UI 主机位于 `http://<gateway-host>:18793/__zoidbergbot__/a2ui/`。
