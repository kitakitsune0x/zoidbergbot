...
- 你仍然可以使用 [Gateway网关健康检查](/gateway/health) 中的 CLI 流程（`zoidbergbot status`、`zoidbergbot status --deep`、`openclaw health --json`），并跟踪 `/tmp/zoidbergbot/zoidbergbot-*.log` 中的 `web-heartbeat` / `web-reconnect`。
