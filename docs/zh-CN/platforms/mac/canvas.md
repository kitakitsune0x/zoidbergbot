...
- `~/Library/Application Support/ZoidbergBot/canvas/<session>/...`

Canvas 面板通过**自定义 URL scheme** 提供这些文件：

- `zoidbergbot-canvas://<session>/<path>`

示例：

- `zoidbergbot-canvas://main/` → `<canvasRoot>/main/index.html`
- `zoidbergbot-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `zoidbergbot-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
zoidbergbot nodes canvas present --node <id>
zoidbergbot nodes canvas navigate --node <id> --url "/"
zoidbergbot nodes canvas eval --node <id> --js "document.title"
zoidbergbot nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__zoidbergbot__/a2ui/
```
...
zoidbergbot nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速冒烟测试：

```bash
zoidbergbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "zoidbergbot://agent?message=Review%20this%20design";
```
