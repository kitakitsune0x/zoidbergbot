...
read_when:
  - 编辑 IPC 协议或菜单栏应用 IPC
summary: ZoidbergBot 应用、gateway 节点传输和 PeekabooBridge 的 macOS IPC 架构
title: macOS IPC
x-i18n:
...
# ZoidbergBot macOS IPC 架构

**当前模型：** 本地 Unix 套接字将**节点宿主服务**连接到 **macOS 应用**，用于执行审批和 `system.run`。存在一个 `zoidbergbot-mac` 调试 CLI 用于发现/连接检查；智能体操作仍通过 Gateway网关 WebSocket 和 `node.invoke` 传递。UI 自动化使用 PeekabooBridge。
...
- 宿主优先级顺序（客户端侧）：Peekaboo.app → Claude.app → ZoidbergBot.app → 本地执行。
