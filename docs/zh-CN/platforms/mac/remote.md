...
read_when:
  - 设置或调试远程 Mac 控制
summary: macOS 应用通过 SSH 远程控制 ZoidbergBot Gateway网关的流程
title: 远程控制
x-i18n:
...
# 远程 ZoidbergBot（macOS ⇄ 远程主机）

此流程让 macOS 应用充当运行在另一台主机（桌面/服务器）上的 ZoidbergBot Gateway网关的完整远程控制器。这是应用的 **Remote over SSH**（远程运行）功能。所有功能——健康检查、语音唤醒转发和 Web Chat——都复用 _设置 → 通用_ 中相同的远程 SSH 配置。

## 模式

- **本地（此 Mac）**：所有内容在笔记本电脑上运行，无需 SSH。
- **Remote over SSH（默认）**：ZoidbergBot 命令在远程主机上执行。Mac 应用使用 `-o BatchMode` 加上你选择的身份/密钥打开 SSH 连接，并进行本地端口转发。
...
1. 安装 Node + pnpm 并构建/安装 ZoidbergBot CLI（`pnpm install && pnpm build && pnpm link --global`）。
2. 确保 `zoidbergbot` 在非交互式 shell 的 PATH 中（如有需要，创建符号链接到 `/usr/local/bin` 或 `/opt/homebrew/bin`）。
3. 开启 SSH 密钥认证。我们建议使用 **Tailscale** IP 以实现局域网外的稳定可达性。

## macOS 应用设置

1. 打开 _设置 → 通用_。
2. 在 **ZoidbergBot runs** 下，选择 **Remote over SSH** 并设置：
   ...
   - **CLI 路径**（高级）：可选的 `zoidbergbot` 可执行入口/二进制文件路径（广播时自动填充）。
3. 点击 **测试远程连接**。成功表示远程 `zoidbergbot status --json` 正常运行。失败通常意味着 PATH/CLI 问题；exit 127 表示远程未找到 CLI。
...
- 在**远程主机**上运行 `zoidbergbot channels login --verbose`。用手机上的 WhatsApp 扫描二维码。
- 如果认证过期，在该主机上重新运行登录。健康检查会显示链接问题。

## 故障排除

- **exit 127 / not found**：`zoidbergbot` 不在非登录 shell 的 PATH 中。将其添加到 `/etc/paths`、你的 shell rc 文件中，或创建符号链接到 `/usr/local/bin`/`/opt/homebrew/bin`。
- **健康探测失败**：检查 SSH 可达性、PATH，以及 Baileys 是否已登录（`zoidbergbot status --json`）。
...
通过 `zoidbergbot` 和 `node.invoke` 在脚本中为每个通知选择声音，例如：

```bash
zoidbergbot nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
