...
summary: ZoidbergBot macOS 伴侣应用（菜单栏 + Gateway网关代理）
title: macOS 应用
x-i18n:
...
# ZoidbergBot macOS 伴侣应用（菜单栏 + Gateway网关代理）

macOS 应用是 ZoidbergBot 的**菜单栏伴侣**。它拥有权限，在本地管理/连接 Gateway网关（launchd 或手动），并将 macOS 能力作为节点暴露给智能体。
...
- 根据请求通过 npm/pnpm 安装全局 CLI（`zoidbergbot`）（不建议将 bun 用于 Gateway网关运行时）。

## 本地模式与远程模式

- **本地**（默认）：如果存在正在运行的本地 Gateway网关，应用会连接到它；否则通过 `zoidbergbot gateway install` 启用 launchd 服务。
...
应用管理一个标签为 `bot.molt.gateway` 的用户级 LaunchAgent（使用 `--profile`/`ZOIDBERGBOT_PROFILE` 时为 `bot.molt.<profile>`；旧版 `com.zoidbergbot.*` 仍会被卸载）。
...
如果 LaunchAgent 未安装，可从应用中启用或运行 `zoidbergbot gateway install`。
...
```
~/.zoidbergbot/exec-approvals.json
```
...
```bash
open 'zoidbergbot://agent?message=Hello%20from%20deep%20link'
```
...
```bash
cd apps/macos
swift run zoidbergbot-mac connect --json
swift run zoidbergbot-mac discover --timeout 3000 --json
```
...
提示：与 `zoidbergbot gateway discover --json` 对比，查看 macOS 应用的发现管道（NWBrowser + tailnet DNS-SD 回退）是否与 Node CLI 基于 `dns-sd` 的发现有差异。
