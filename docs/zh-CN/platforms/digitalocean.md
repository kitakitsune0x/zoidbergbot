---
read_when:
  - 在 DigitalOcean 上设置 ZoidbergBot
  - 寻找便宜的 VPS 托管来运行 ZoidbergBot
summary: 在 DigitalOcean 上运行 ZoidbergBot（简单的付费 VPS 方案）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 ZoidbergBot

## 目标

在 DigitalOcean 上运行持久化的 ZoidbergBot Gateway网关，费用为**每月 $6**（预留定价为每月 $4）。
...
- `~/.zoidbergbot/workspace/` — 工作区（SOUL.md、记忆等）
