...
- 通过 `zoidbergbot message send --media` 发送带可选说明文字的媒体。
- 允许来自 Web 收件箱的自动回复在文本旁包含媒体。
- 保持每种类型的限制合理且可预测。

## CLI 接口

- `zoidbergbot message send --media <path-or-url> [--message <caption>]`
...
- 当存在媒体时，Web 发送器使用与 `zoidbergbot message send` 相同的管道解析本地路径或 URL。
- 如果提供了多个媒体条目，则按顺序依次发送。

## 入站媒体转命令 (Pi)

- 当入站 Web 消息包含媒体时，ZoidbergBot 会下载到临时文件并暴露模板变量：
  - `{{MediaUrl}}` 入站媒体的伪 URL。
