...
```bash
zoidbergbot nodes camera snap --node <id>               # 默认：前后摄像头都拍摄（2 行 MEDIA 输出）
zoidbergbot nodes camera snap --node <id> --facing front
zoidbergbot nodes camera clip --node <id> --duration 3000
zoidbergbot nodes camera clip --node <id> --no-audio
```
...
- **设置 → 通用 → 允许相机**（`zoidbergbot.cameraEnabled`）
  - 默认：**关闭**
  - 关闭时：相机请求返回"Camera disabled by user"。

### CLI 辅助工具（节点调用）

使用主 `zoidbergbot` CLI 在 macOS 节点上调用相机命令。

示例：

```bash
zoidbergbot nodes camera list --node <id>            # 列出相机 ID
zoidbergbot nodes camera snap --node <id>            # 输出 MEDIA:<path>
zoidbergbot nodes camera snap --node <id> --max-width 1280
zoidbergbot nodes camera snap --node <id> --delay-ms 2000
zoidbergbot nodes camera snap --node <id> --device-id <id>
zoidbergbot nodes camera clip --node <id> --duration 10s          # 输出 MEDIA:<path>
zoidbergbot nodes camera clip --node <id> --duration-ms 3000      # 输出 MEDIA:<path>（旧版标志）
zoidbergbot nodes camera clip --node <id> --device-id <id>
zoidbergbot nodes camera clip --node <id> --no-audio
```

注意事项：

- `zoidbergbot nodes camera snap` 默认 `maxWidth=1600`，除非被覆盖。
...
```bash
zoidbergbot nodes screen record --node <id> --duration 10s --fps 15   # 输出 MEDIA:<path>
```
