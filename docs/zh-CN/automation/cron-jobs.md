...
- 任务持久化存储在 `~/.zoidbergbot/cron/` 下，因此重启不会丢失计划。
- 两种执行方式：
...
zoidbergbot cron list
zoidbergbot cron run <job-id> --force
zoidbergbot cron runs --id <job-id>
```
...
定时任务默认持久化存储在 Gateway网关主机的 `~/.zoidbergbot/cron/jobs.json` 中。Gateway网关将文件加载到内存中，并在更改时写回，因此仅在 Gateway网关停止时手动编辑才是安全的。请优先使用 `zoidbergbot cron add/edit` 或定时任务工具调用 API 进行更改。
...
- 任务存储：`~/.zoidbergbot/cron/jobs.json`（Gateway网关管理的 JSON）。
- 运行历史：`~/.zoidbergbot/cron/runs/<jobId>.jsonl`（JSONL，自动清理）。
...
```json5
{
  cron: {
    ...
    store: "~/.zoidbergbot/cron/jobs.json",
...
zoidbergbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# 切换或清除现有任务的智能体
zoidbergbot cron edit <jobId> --agent ops
zoidbergbot cron edit <jobId> --clear-agent
```
...
- 检查定时任务是否已启用：`cron.enabled` 和 `ZOIDBERGBOT_SKIP_CRON`。
