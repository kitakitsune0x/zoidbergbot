---
read_when:
  - 将 Gmail 收件箱触发器接入 ZoidbergBot
  - 为智能体唤醒设置 Pub/Sub 推送
summary: 通过 gogcli 将 Gmail Pub/Sub 推送接入 ZoidbergBot webhooks
title: Gmail PubSub
x-i18n:
...
# Gmail Pub/Sub -> ZoidbergBot

目标：Gmail watch -> Pub/Sub 推送 -> `gog gmail watch serve` -> ZoidbergBot webhook。
...
- 已启用 ZoidbergBot hooks（参见 [Webhooks](/automation/webhook)）。
...
使用 ZoidbergBot 辅助工具一键完成所有配置（在 macOS 上通过 brew 安装依赖）：

```bash
zoidbergbot webhooks gmail setup \
...
- 为 `zoidbergbot webhooks gmail run` 写入 `hooks.gmail` 配置。
- 启用 Gmail hook 预设（`hooks.presets: ["gmail"]`）。

路径说明：当启用 `tailscale.mode` 时，ZoidbergBot 会自动将 `hooks.gmail.serve.path` 设置为 `/`，并将公共路径保持在 `hooks.gmail.tailscale.path`（默认 `/gmail-pubsub`），因为 Tailscale 在代理前会去除设置的路径前缀。
...
gog gmail watch serve \
  ...
  --hook-token ZOIDBERGBOT_HOOK_TOKEN \
  --include-body \
  --max-bytes 20000
```
...
- `--hook-url` 指向 ZoidbergBot `/hooks/gmail`（已映射；隔离运行 + 摘要发送到主会话）。
- `--include-body` 和 `--max-bytes` 控制发送到 ZoidbergBot 的正文片段。

推荐：`zoidbergbot webhooks gmail run` 封装了相同的流程并自动续期 watch。
...
```bash
gog gmail watch status --account zoidbergbot@gmail.com
gog gmail history --account zoidbergbot@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account zoidbergbot@gmail.com
