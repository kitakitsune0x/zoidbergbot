...
summary: ZoidbergBot 最高风险路径的机器验证安全模型。
title: 形式化验证（安全模型）
x-i18n:
...
**目标（北极星）：** 提供机器验证的论证，证明 ZoidbergBot 在明确假设下执行了其预期的安全策略（授权、会话隔离、工具门控和错误配置安全性）。
...
**尚未实现的：** 证明"ZoidbergBot 在所有方面都是安全的"或完整的 TypeScript 实现是正确的。
