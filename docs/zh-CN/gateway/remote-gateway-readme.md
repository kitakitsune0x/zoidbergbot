...
summary: 为 ZoidbergBot.app 连接远程 Gateway网关设置 SSH 隧道
...
# 使用远程 Gateway网关运行 ZoidbergBot.app

ZoidbergBot.app 使用 SSH 隧道连接到远程 Gateway网关。本指南将介绍如何进行设置。
...
│                        客户端机器                              │
│                                                              │
│  ZoidbergBot.app ──► ws://127.0.0.1:18789（本地端口）              │
│                     │                                        │
│                     ▼                                        │
│  SSH 隧道 ──────────────────────────────────────────────────│
│                     │                                        │
...
```bash
launchctl setenv ZOIDBERGBOT_GATEWAY_TOKEN "<your-token>"
```
...
### 第 5 步：重启 ZoidbergBot.app

```bash
# 退出 ZoidbergBot.app（⌘Q），然后重新打开：
open /path/to/ZoidbergBot.app
```
...
旧版说明：如果存在残留的 `com.zoidbergbot.ssh-tunnel` LaunchAgent，请将其删除。
...
ZoidbergBot.app 连接到客户端机器上的 `ws://127.0.0.1:18789`。SSH 隧道将该连接转发到运行 Gateway网关的远程机器的 18789 端口。
