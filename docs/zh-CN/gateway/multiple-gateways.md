...
summary: 在同一主机上运行多个 ZoidbergBot Gateway网关（隔离、端口和配置文件）
title: 多个 Gateway网关
x-i18n:
...
- `ZOIDBERGBOT_CONFIG_PATH` — 每个实例独立的配置文件
- `ZOIDBERGBOT_STATE_DIR` — 每个实例独立的会话、凭据、缓存
...
配置文件会自动限定 `ZOIDBERGBOT_STATE_DIR` + `ZOIDBERGBOT_CONFIG_PATH` 的作用域，并为服务名称添加后缀。

```bash
# 主实例
zoidbergbot --profile main setup
zoidbergbot --profile main gateway --port 18789

# 救援实例
zoidbergbot --profile rescue setup
zoidbergbot --profile rescue gateway --port 19001
```
...
# 救援机器人（独立配置文件 + 端口）
zoidbergbot --profile rescue onboard
# 注意：
...
```bash
ZOIDBERGBOT_CONFIG_PATH=~/.zoidbergbot/main.json \
ZOIDBERGBOT_STATE_DIR=~/.zoidbergbot-main \
zoidbergbot gateway --port 18789

ZOIDBERGBOT_CONFIG_PATH=~/.zoidbergbot/rescue.json \
ZOIDBERGBOT_STATE_DIR=~/.zoidbergbot-rescue \
...
```bash
zoidbergbot --profile main status
zoidbergbot --profile rescue status
zoidbergbot --profile rescue browser status
```
