...
ZoidbergBot 可以为 Gateway网关仪表盘和 WebSocket 端口自动配置 Tailscale **Serve**（tailnet）或 **Funnel**（公网）。这样 Gateway网关仍然绑定在 local loopback 上，而由 Tailscale 提供 HTTPS、路由以及（对于 Serve）身份头信息。

## 模式

- `serve`：通过 `tailscale serve` 仅限 Tailnet 的 Serve。Gateway网关保持在 `127.0.0.1` 上。
- `funnel`：通过 `tailscale funnel` 提供公网 HTTPS。ZoidbergBot 要求设置共享密码。
...
- `token`（设置了 `ZOIDBERGBOT_GATEWAY_TOKEN` 时的默认值）
- `password`（通过 `ZOIDBERGBOT_GATEWAY_PASSWORD` 或配置文件设置的共享密钥）

当 `tailscale.mode = "serve"` 且 `gateway.auth.allowTailscale` 为 `true` 时，有效的 Serve 代理请求可以通过 Tailscale 身份头（`tailscale-user-login`）进行认证，无需提供令牌/密码。ZoidbergBot 通过本地 Tailscale 守护进程（`tailscale whois`）解析 `x-forwarded-for` 地址，并将其与头信息匹配来验证身份。ZoidbergBot 仅在请求来自 local loopback 且带有 Tailscale 的 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host` 头时，才将其视为 Serve 请求。
...
建议使用 `ZOIDBERGBOT_GATEWAY_PASSWORD` 而非将密码提交到磁盘。

## CLI 示例

```bash
zoidbergbot gateway --tailscale serve
zoidbergbot gateway --tailscale funnel --auth password
```
...
- 如果你希望 ZoidbergBot 在关闭时撤销 `tailscale serve` 或 `tailscale funnel` 配置，请设置 `gateway.tailscale.resetOnExit`。
