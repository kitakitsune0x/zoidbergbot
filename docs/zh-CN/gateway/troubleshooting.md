...
read_when:
  - 排查运行时问题或故障时
summary: 常见 ZoidbergBot 故障的快速解决问题指南
title: 故障排除
x-i18n:
...
| 命令                               | 告诉你什么                                                                           | 何时使用                         |
| ---------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------- |
| `zoidbergbot status`                  | 本地摘要：操作系统 + 更新、Gateway网关可达性/模式、服务、智能体/会话、提供商配置状态 | 首次检查，快速概览               |
| `zoidbergbot status --all`            | 完整本地诊断（只读、可粘贴、基本安全）包含日志尾部                                   | 需要分享调试报告时               |
| `zoidbergbot status --deep`           | 运行 Gateway网关健康检查（包括提供商探测；需要 Gateway网关可达）                     | 当"已配置"不等于"正常工作"时     |
| `zoidbergbot gateway probe`           | Gateway网关发现 + 可达性（本地 + 远程目标）                                          | 怀疑探测了错误的 Gateway网关时   |
| `zoidbergbot channels status --probe` | 向运行中的 Gateway网关查询渠道状态（可选探测）                                       | Gateway网关可达但渠道异常时      |
| `zoidbergbot gateway status`          | 管理器状态（launchd/systemd/schtasks）、运行时 PID/退出码、最后一次 Gateway网关错误  | 服务"看起来已加载"但实际未运行时 |
| `zoidbergbot logs --follow`           | 实时日志（运行时问题的最佳信号源）                                                   | 需要查看实际失败原因时           |

**分享输出：** 优先使用 `zoidbergbot status --all`（它会脱敏令牌）。如果粘贴 `zoidbergbot status` 的输出，建议先设置 `ZOIDBERGBOT_SHOW_SECRETS=0`（令牌预览）。
...
- 推荐：`zoidbergbot logs --follow`
- 文件日志（始终可用）：`/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log`（或你配置的 `logging.file`）
- macOS LaunchAgent（如已安装）：`$ZOIDBERGBOT_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`
- Linux systemd（如已安装）：`journalctl --user -u zoidbergbot-gateway[-<profile>].service -n 200 --no-pager`
...
变量如 `DISPLAY` 应放在 `~/.zoidbergbot/.env` 中（由 Gateway网关在启动早期加载）。
...
服务安装时使用了 Bun 或版本管理器管理的 Node 路径，请运行 `zoidbergbot doctor`
以迁移到系统级 Node 安装。
...
- 或将密钥内置到自定义沙箱镜像中
- 然后运行 `zoidbergbot sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 对于 `zoidbergbot gateway` 和服务必须为 `local`。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `zoidbergbot gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `zoidbergbot gateway status` 和 `zoidbergbot doctor` 会在服务看起来正在运行但端口未打开时显示**最后一次 Gateway网关错误**日志。
- 非 local loopback 绑定（`lan`/`tailnet`/`custom`，或 local loopback 不可用时的 `auto`）需要认证：
  `gateway.auth.token`（或 `ZOIDBERGBOT_GATEWAY_TOKEN`）。
- `gateway.remote.token` 仅用于远程 CLI 调用；它**不会**启用本地认证。
- `gateway.token` 会被忽略；请使用 `gateway.auth.token`。

**如果 `zoidbergbot gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该一致。
- 如果不一致，几乎可以确定你在编辑一个配置而服务在运行另一个配置。
- 修复：从你希望服务使用的相同 `--profile` / `ZOIDBERGBOT_STATE_DIR` 重新运行 `zoidbergbot gateway install --force`。

**如果 `zoidbergbot gateway status` 报告服务配置问题**

- 管理器配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `zoidbergbot doctor` 更新配置（或 `zoidbergbot gateway install --force` 完整重写）。
...
- 修复：设置 `gateway.auth.mode` + `gateway.auth.token`（或导出 `ZOIDBERGBOT_GATEWAY_TOKEN`）并重启服务。

**如果 `zoidbergbot gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
症状：`pwd` 或文件工具显示 `~/.zoidbergbot/sandboxes/...`，但你
期望的是主机工作区。
...
ZoidbergBot 有意拒绝**旧版/不安全的模型**（特别是那些更
...
- 如果不确定有哪些可用模型，运行 `zoidbergbot models list` 或
  `zoidbergbot models scan` 并选择一个受支持的模型。
...
trash "${ZOIDBERGBOT_STATE_DIR:-$HOME/.openclaw}/credentials" # 如果 logout 无法完全清除
zoidbergbot channels login --verbose       # 重新扫描二维码
```
...
ZoidbergBot 现在会在配置包含未知键、格式错误的值或无效类型时拒绝启动。
这是出于安全考虑的有意设计。
...
- `zoidbergbot doctor` 会报告每个无效条目。
- `zoidbergbot doctor --fix` 会应用迁移/修复并重写配置。
- 诊断命令如 `zoidbergbot logs`、`openclaw health`、`zoidbergbot status`、`zoidbergbot gateway status` 和 `zoidbergbot gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该先检查什么？

- 正在使用的提供商是否存在**凭据**（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退模型是你能访问的模型。
- `/tmp/zoidbergbot/…` 中的 **Gateway网关日志**查看具体的提供商错误。
- **模型状态**：使用 `/model status`（聊天中）或 `zoidbergbot models status`（CLI）。
...
5. 运行 `zoidbergbot channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。ZoidbergBot 在当前 `main` 中会清洗/规范化工具
...
如果重置无效，更改 [`scripts/package-mac-app.sh`](https://github.com/zoidbergbot/zoidbergbot/blob/main/scripts/package-mac-app.sh) 中的 `BUNDLE_ID`（例如添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
# 或：launchctl bootout gui/$UID/bot.molt.gateway（替换为 bot.molt.<profile>；旧版 com.zoidbergbot.* 仍可用）
```
...
**修复 3：检查 CLI 安装**
确保全局 `zoidbergbot` CLI 已安装且版本与应用匹配：
...
| 日志                          | 位置                                                                                                                                                                                                                                                                                                                          |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway网关文件日志（结构化） | `/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log`（或 `logging.file`）                                                                                                                                                                                                                                                                  |
| Gateway网关服务日志（管理器） | macOS：`$ZOIDBERGBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.zoidbergbot/logs/...`；profile 使用 `~/.zoidbergbot-<profile>/logs/...`）<br />Linux：`journalctl --user -u zoidbergbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "ZoidbergBot Gateway网关 (<profile>)" /V /FO LIST` |
| 会话文件                      | `$ZOIDBERGBOT_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                              |
| 媒体缓存                      | `$ZOIDBERGBOT_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                  |
| 凭据                          | `$ZOIDBERGBOT_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                            |
...
zoidbergbot channels login         # 重新配对 WhatsApp
zoidbergbot gateway restart           # 或：zoidbergbot gateway
```
...
1. 先查看日志：`/tmp/zoidbergbot/`（默认：`zoidbergbot-YYYY-MM-DD.log`，或你配置的 `logging.file`）
