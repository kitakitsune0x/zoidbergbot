...
ZoidbergBot 支持通过 OAuth 和 API 密钥对模型提供商进行认证。对于 Anthropic 账户，我们推荐使用 **API 密钥**。对于 Claude 订阅访问，请使用 `claude setup-token` 创建的长期有效令牌。
...
2. 将其放置在 **Gateway网关主机**（运行 `zoidbergbot gateway` 的机器）上。
...
3. 如果 Gateway网关在 systemd/launchd 下运行，建议将密钥放在 `~/.zoidbergbot/.env` 中，以便守护进程能够读取：

```bash
cat >> ~/.zoidbergbot/.env <<'EOF'
ANTHROPIC_API_KEY=...
EOF
```
...
参见[帮助](/help)了解环境变量继承的详细信息（`env.shellEnv`、`~/.zoidbergbot/.env`、systemd/launchd）。
...
```bash
zoidbergbot models auth paste-token --provider anthropic
zoidbergbot models auth paste-token --provider openrouter
```

适用于自动化的检查（过期/缺失时退出码为 `1`，即将过期时为 `2`）：

```bash
zoidbergbot models status --check
```
...
运行 `zoidbergbot models status` 确认哪个配置即将过期。如果配置缺失，请重新运行 `claude setup-token` 并再次粘贴令牌。
