...
ZoidbergBot 可以将**本地 AI CLI** 作为**纯文本回退**运行，适用于 API 提供商宕机、被限流或暂时异常的情况。该方案设计上有意保持保守：
...
你可以**无需任何配置**直接使用 Claude Code CLI（ZoidbergBot 内置了默认配置）：

```bash
zoidbergbot agent --message "hi" --model claude-cli/opus-4.5
```

Codex CLI 同样开箱即用：

```bash
zoidbergbot agent --message "hi" --model codex-cli/gpt-5.2-codex
```
...
- 如果主提供商失败（认证、限流、超时），ZoidbergBot 将尝试使用 CLI 后端。
...
2. **构建系统提示词**，使用相同的 ZoidbergBot 提示词 + 工作区上下文。
...
ZoidbergBot 会将 base64 图片写入临时文件。如果设置了 `imageArg`，这些路径将作为 CLI 参数传递。如果未设置 `imageArg`，ZoidbergBot 会将文件路径追加到提示词中（路径注入），这对于能从纯文本路径自动加载本地文件的 CLI 已经足够（Claude Code CLI 的行为即是如此）。
...
ZoidbergBot 为 `claude-cli` 内置了默认配置：
...
ZoidbergBot 还为 `codex-cli` 内置了默认配置：
...
- **无 ZoidbergBot 工具**（CLI 后端不会接收工具调用）。某些 CLI 可能仍会运行自身的智能体工具。
- **无流式传输**（CLI 输出收集完毕后一次性返回）。
- **结构化输出**取决于 CLI 的 JSON 格式。
- **Codex CLI 会话**通过文本输出恢复（非 JSONL），其结构化程度不如初始的 `--json` 运行。ZoidbergBot 会话仍正常工作。
