...
- 使用 macOS 应用的**通过 SSH 远程连接**模式（设置 → 通用 → "ZoidbergBot 运行位置"）。
...
- `openclaw health` 和 `zoidbergbot status --deep` 现在通过 `ws://127.0.0.1:18789` 访问远程 Gateway网关。
- `zoidbergbot gateway {status,health,send,agent,call}` 也可以在需要时通过 `--url` 指向转发的 URL。

注意：将 `18789` 替换为你配置的 `gateway.port`（或 `--port`/`ZOIDBERGBOT_GATEWAY_PORT`）。
