...
status: active
summary: ZoidbergBot 沙箱的工作原理：模式、作用域、工作区访问和镜像
title: 沙箱
x-i18n:
...
ZoidbergBot 可以**在 Docker 容器内运行工具**以缩小影响范围。
...
- `"none"`（默认）：工具在 `~/.zoidbergbot/sandboxes` 下的沙箱工作区中运行。
...
ZoidbergBot 会将符合条件的 Skills 镜像到沙箱工作区（`.../skills`）中以便读取。当设为 `"rw"` 时，工作区 Skills 可从
`/workspace/skills` 读取。
...
默认镜像：`zoidbergbot-sandbox:bookworm-slim`
...
- 使用 `zoidbergbot sandbox explain` 检查当前生效的沙箱模式、工具策略和修复配置项。
