...
Gateway网关 WebSocket 协议是 ZoidbergBot 的**统一控制平面 + 节点传输层**。所有客户端（CLI、Web UI、macOS 应用、iOS/Android 节点、无头节点）均通过 WebSocket 连接，并在握手时声明其**角色** + **作用域**。
...
```json
{
  ...
  "params": {
    ...
    "auth": { "token": "…" },
    "locale": "en-US",
    "userAgent": "zoidbergbot-cli/1.2.3",
    "device": {
...
```json
{
  ...
  "params": {
    ...
    "auth": { "token": "…" },
    "locale": "en-US",
    "userAgent": "zoidbergbot-ios/1.2.3",
    "device": {
      "id": "device_fingerprint",
      "publicKey": "…",
      "signature": "…",
...
- 如果设置了 `ZOIDBERGBOT_GATEWAY_TOKEN`（或 `--token`），`connect.params.auth.token` 必须匹配，否则连接将被关闭。
