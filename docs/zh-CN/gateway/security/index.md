...
## 快速检查：`zoidbergbot security audit`
...
- 收紧本地权限（`~/.zoidbergbot` → `700`，配置文件 → `600`，以及常见状态文件如 `credentials/*.json`、`agents/*/agent/auth-profiles.json` 和 `agents/*/sessions/sessions.json`）。

在你的机器上运行具有 shell 访问权限的 AI 智能体是……_相当刺激的_。以下是如何避免被攻破的方法。

ZoidbergBot 既是一个产品也是一个实验：你正在将前沿模型的行为接入真实的消息平台和真实的工具。**不存在"完美安全"的配置。** 目标是有意识地控制：
...
如果运行 `--deep`，ZoidbergBot 还会尝试对 Gateway网关进行尽力而为的实时探测。
...
- **配对允许列表**：`~/.zoidbergbot/credentials/<channel>-allowFrom.json`
- **模型认证配置**：`~/.zoidbergbot/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.zoidbergbot/credentials/oauth.json`
...
`zoidbergbot security audit` 会在此设置启用时发出警告。
...
ZoidbergBot 将会话记录存储在 `~/.zoidbergbot/agents/<agentId>/sessions/*.jsonl` 目录下。这是会话连续性和（可选的）会话记忆索引所必需的，但这也意味着**任何具有文件系统访问权限的进程/用户都可以读取这些日志**。将磁盘访问视为信任边界，并锁定 `~/.zoidbergbot` 的权限（参见下方的审计部分）。如果你需要智能体之间更强的隔离，请在不同的操作系统用户或不同的主机上运行它们。
...
ZoidbergBot 可以在会话中刷新 Skills 列表：
...
- 启用前检查插件配置。
- 插件更改后重启 Gateway网关。
- 如果你从 npm 安装插件（`zoidbergbot plugins install <npm-spec>`），请视同运行不受信任的代码：
  - 安装路径为 `~/.zoidbergbot/extensions/<pluginId>/`（或 `$ZOIDBERGBOT_STATE_DIR/extensions/<pluginId>/`）。
  - ZoidbergBot 使用 `npm pack` 然后在该目录中运行 `npm install --omit=dev`（npm 生命周期脚本可以在安装期间执行代码）。
...
默认情况下，ZoidbergBot 将**所有私聊路由到主会话**，以便你的助手在设备和渠道之间保持连续性。如果**多人**可以私聊机器人（开放私聊或多人允许列表），请考虑隔离私聊会话：
...
ZoidbergBot 有两个独立的"谁可以触发我？"层级：

- **私聊允许列表**（`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`）：谁可以在私聊中与机器人对话。
  - 当 `dmPolicy="pairing"` 时，批准记录写入 `~/.zoidbergbot/credentials/<channel>-allowFrom.json`（与配置允许列表合并）。
...
4. **重新运行审计**
   - `zoidbergbot security audit --deep` 并确认报告是干净的。
...
- `~/.zoidbergbot/zoidbergbot.json`：`600`（仅用户可读写）
- `~/.zoidbergbot`：`700`（仅用户）

`zoidbergbot doctor` 可以警告并提供收紧这些权限的选项。
...
- 默认：`18789`
- 配置/标志/环境变量：`gateway.port`、`--port`、`ZOIDBERGBOT_GATEWAY_PORT`
...
Gateway网关通过 mDNS（端口 5353 上的 `_zoidbergbot-gw._tcp`）广播其存在以供本地设备发现。在完整模式下，这包括可能暴露运营细节的 TXT 记录：
...
4. **环境变量**（替代方案）：设置 `ZOIDBERGBOT_DISABLE_BONJOUR=1` 以在不更改配置的情况下禁用 mDNS。
...
Doctor 可以为你生成一个：`zoidbergbot doctor --generate-gateway-token`。
...
- `gateway.auth.mode: "password"`：密码认证（优先通过环境变量设置：`ZOIDBERGBOT_GATEWAY_PASSWORD`）。

轮换检查清单（令牌/密码）：

1. 生成/设置新密钥（`gateway.auth.token` 或 `ZOIDBERGBOT_GATEWAY_PASSWORD`）。
...
当 `gateway.auth.allowTailscale` 为 `true`（Serve 的默认值）时，ZoidbergBot 接受 Tailscale Serve 身份头（`tailscale-user-login`）作为认证。ZoidbergBot 通过本地 Tailscale 守护进程（`tailscale whois`）解析 `x-forwarded-for` 地址并将其与头匹配来验证身份。这仅在请求命中 local loopback 且包含由 Tailscale 注入的 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host` 时触发。
...
- ZoidbergBot 将信任来自这些 IP 的 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
...
假设 `~/.zoidbergbot/`（或 `$ZOIDBERGBOT_STATE_DIR/`）下的任何内容都可能包含密钥或私有数据：

- `zoidbergbot.json`：配置可能包含令牌（Gateway网关、远程 Gateway网关）、提供商设置和允许列表。
...
- 分享诊断信息时，优先使用 `zoidbergbot status --all`（可粘贴，密钥已脱敏）而非原始日志。
...
- `agents.defaults.sandbox.workspaceAccess: "none"`（默认）使智能体工作区不可访问；工具在 `~/.zoidbergbot/sandboxes` 下的沙箱工作区中运行
...
- 优先为智能体使用专用配置文件（默认的 `zoidbergbot` 配置文件）。
...
- Chrome 扩展中继的 CDP 端点有认证保护；只有 ZoidbergBot 客户端可以连接。
...
1. **停止它：** 停止 macOS 应用（如果它管理 Gateway网关）或终止你的 `zoidbergbot gateway` 进程。
...
1. 轮换 Gateway网关认证（`gateway.auth.token` / `ZOIDBERGBOT_GATEWAY_PASSWORD`）并重启。
...
1. 检查 Gateway网关日志：`/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log`（或 `logging.file`）。
2. 审查相关的记录：`~/.zoidbergbot/agents/<agentId>/sessions/*.jsonl`。
3. 审查最近的配置更改（任何可能扩大访问的更改：`gateway.bind`、`gateway.auth`、私聊/群组策略、`tools.elevated`、插件更改）。

### 收集报告

- 时间戳、Gateway网关主机操作系统 + ZoidbergBot 版本
- 会话记录 + 简短的日志尾部（脱敏后）
- 攻击者发送了什么 + 智能体做了什么
...
发现了 ZoidbergBot 的漏洞？请负责任地报告：

1. 邮箱：security@zoidbergbot.ai
2. 修复之前请勿公开发布
3. 我们会致谢你（除非你希望匿名）
