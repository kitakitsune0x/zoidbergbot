...
ZoidbergBot 的 Gateway网关可以提供一个兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为普通的 Gateway网关智能体运行来执行（与 `zoidbergbot agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway网关一致。
...
- 当 `gateway.auth.mode="token"` 时，使用 `gateway.auth.token`（或 `ZOIDBERGBOT_GATEWAY_TOKEN`）。
- 当 `gateway.auth.mode="password"` 时，使用 `gateway.auth.password`（或 `ZOIDBERGBOT_GATEWAY_PASSWORD`）。

## 选择智能体

无需自定义头：在 OpenResponses 的 `model` 字段中编码智能体 ID：

- `model: "zoidbergbot:<agentId>"`（示例：`"zoidbergbot:main"`、`"zoidbergbot:beta"`）
