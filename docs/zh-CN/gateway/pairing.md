...
zoidbergbot nodes reject <requestId>
zoidbergbot nodes status
zoidbergbot nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
配对状态存储在 Gateway网关状态目录下（默认 `~/.zoidbergbot`）：

- `~/.zoidbergbot/nodes/paired.json`
- `~/.zoidbergbot/nodes/pending.json`

如果你覆盖了 `ZOIDBERGBOT_STATE_DIR`，`nodes/` 文件夹会随之移动。
