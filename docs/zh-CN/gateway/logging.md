...
- 默认滚动日志文件位于 `/tmp/zoidbergbot/` 下（每天一个文件）：`zoidbergbot-YYYY-MM-DD.log`
  - 日期使用 Gateway网关主机的本地时区。
- 日志文件路径和级别可通过 `~/.zoidbergbot/zoidbergbot.json` 配置：
  - `logging.file`
  - `logging.level`
...
`zoidbergbot gateway` 支持按 Gateway网关设置的样式开关：
...
# 显示所有 WS 流量（配对）
zoidbergbot gateway --verbose --ws-log compact

# 显示所有 WS 流量（完整元数据）
zoidbergbot gateway --verbose --ws-log full
```
