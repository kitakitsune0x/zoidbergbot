...
- 替代旧版 `gateway` 命令。CLI 入口：`zoidbergbot gateway`。
...
# 如果端口被占用，先终止监听进程再启动：
zoidbergbot gateway --force
# 开发循环（TS 文件变更时自动重载）：
pnpm gateway:watch
```

- 配置热重载会监视 `~/.zoidbergbot/zoidbergbot.json`（或 `ZOIDBERGBOT_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）启动 Canvas 文件服务器，从 `~/.zoidbergbot/workspace/canvas` 提供 `http://<gateway-host>:18793/__zoidbergbot__/canvas/` 服务。通过 `canvasHost.enabled=false` 或 `ZOIDBERGBOT_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway网关认证：设置 `gateway.auth.token`（或 `ZOIDBERGBOT_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 local loopback 上也是如此。
- 端口优先级：`--port` > `ZOIDBERGBOT_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- macOS：`bot.molt.<profile>`（旧版 `com.zoidbergbot.*` 可能仍然存在）
- Linux：`zoidbergbot-gateway-<profile>.service`
- Windows：`ZoidbergBot Gateway网关 (<profile>)`

安装元数据嵌入在服务配置中：

- `ZOIDBERGBOT_SERVICE_MARKER=openclaw`
- `ZOIDBERGBOT_SERVICE_KIND=gateway`
- `ZOIDBERGBOT_SERVICE_VERSION=<version>`
...
```bash
zoidbergbot --dev setup
zoidbergbot --dev gateway --allow-unconfigured
# 然后指向开发实例：
zoidbergbot --dev status
zoidbergbot --dev health
```

默认值（可通过环境变量/标志/配置覆盖）：

- `ZOIDBERGBOT_STATE_DIR=~/.zoidbergbot-dev`
- `ZOIDBERGBOT_CONFIG_PATH=~/.zoidbergbot-dev/zoidbergbot.json`
- `ZOIDBERGBOT_GATEWAY_PORT=19001`（Gateway网关 WS + HTTP）
- 浏览器控制服务端口 = `19003`（派生：`gateway.port+2`，仅 local loopback）
- `canvasHost.port=19005`（派生：`gateway.port+4`）
- 在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.zoidbergbot/workspace-dev`。

派生端口（经验规则）：

- 基础端口 = `gateway.port`（或 `ZOIDBERGBOT_GATEWAY_PORT` / `--port`）
- 浏览器控制服务端口 = 基础端口 + 2（仅 local loopback）
- `canvasHost.port = 基础端口 + 4`（或 `ZOIDBERGBOT_CANVAS_HOST_PORT` / 配置覆盖）
- 浏览器配置文件 CDP 端口从 `browser.controlPort + 9 .. + 108` 自动分配（按配置文件持久化）。

每个实例的检查清单：

- 唯一的 `gateway.port`
- 唯一的 `ZOIDBERGBOT_CONFIG_PATH`
- 唯一的 `ZOIDBERGBOT_STATE_DIR`
...
```bash
zoidbergbot --profile main gateway install
zoidbergbot --profile rescue gateway install
```

示例：

```bash
ZOIDBERGBOT_CONFIG_PATH=~/.zoidbergbot/a.json ZOIDBERGBOT_STATE_DIR=~/.zoidbergbot-a zoidbergbot gateway --port 19001
ZOIDBERGBOT_CONFIG_PATH=~/.zoidbergbot/b.json ZOIDBERGBOT_STATE_DIR=~/.zoidbergbot-b zoidbergbot gateway --port 19002
```
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，请使用自定义 LaunchDaemon（未随附）。
  - `zoidbergbot gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.zoidbergbot.*` 会被清理）。
  - `zoidbergbot doctor` 审计 LaunchAgent 配置，并可将其更新为当前推荐的默认值。
...
- 如果检测到其他类似 Gateway网关的服务，CLI 会发出警告，除非它们是 ZoidbergBot 配置文件服务。
  我们仍建议大多数场景下**每台机器一个 Gateway网关**；使用隔离的配置文件/端口实现冗余或救援机器人。参见 [多 Gateway网关](/gateway/multiple-gateways)。
  - 清理：`zoidbergbot gateway uninstall`（当前服务）和 `zoidbergbot doctor`（旧版迁移）。
- `gateway install` 在已安装时为空操作；使用 `zoidbergbot gateway install --force` 重新安装（配置文件/环境/路径变更）。

捆绑的 Mac 应用：

- ZoidbergBot.app 可以捆绑一个基于 Node 的 Gateway网关中继，并安装按用户的 LaunchAgent，标签为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.zoidbergbot.*` 标签仍可正常卸载）。
- 要正常停止，使用 `zoidbergbot gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `zoidbergbot gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `zoidbergbot gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 进程监管（systemd 用户单元）

ZoidbergBot 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们推荐单用户机器使用用户服务（更简单的环境，按用户配置）。对于多用户或始终在线的服务器，使用**系统服务**（无需 lingering，共享监管）。

`zoidbergbot gateway install` 写入用户单元。`zoidbergbot doctor` 审计该单元，并可将其更新为当前推荐的默认值。

创建 `~/.config/systemd/user/zoidbergbot-gateway[-<profile>].service`：

```
[Unit]
Description=ZoidbergBot Gateway网关 (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/zoidbergbot gateway --port 18789
Restart=always
RestartSec=5
Environment=ZOIDBERGBOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now zoidbergbot-gateway[-<profile>].service
```

**替代方案（系统服务）** - 对于始终在线或多用户服务器，可以安装 systemd **系统**单元而非用户单元（无需 lingering）。创建 `/etc/systemd/system/zoidbergbot-gateway[-<profile>].service`（复制上述单元，将 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：

```
sudo systemctl daemon-reload
sudo systemctl enable --now zoidbergbot-gateway[-<profile>].service
```
...
- `zoidbergbot gateway health|status` — 通过 Gateway网关 WS 请求健康/状态信息。
- `zoidbergbot message send --target <num> --message "hi" [--media ...]` — 通过 Gateway网关发送（对 WhatsApp 具有幂等性）。
- `zoidbergbot agent --message "hi" --to <num>` — 运行智能体回合（默认等待最终结果）。
- `zoidbergbot gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `zoidbergbot gateway stop|restart` — 停止/重启受监管的 Gateway网关服务（launchd/systemd）。
- Gateway网关辅助子命令假设 Gateway网关已在 `--url` 上运行；它们不再自动启动 Gateway网关。

## 迁移指南

- 弃用 `zoidbergbot gateway` 和旧版 TCP 控制端口的用法。
