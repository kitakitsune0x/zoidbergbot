...
ZoidbergBot 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现活跃的 Gateway网关（WebSocket 端点）。这是尽力而为的机制，**不能**替代 SSH 或基于 Tailnet 的连接。
...
2. 在专用区域下为 `_zoidbergbot-gw._tcp` 发布 DNS‑SD 记录（示例：`zoidbergbot.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该 DNS 服务器为客户端（包括 iOS）解析。

ZoidbergBot 支持任意发现域名；`zoidbergbot.internal.` 仅为示例。iOS/Android 节点会同时浏览 `local.` 和你配置的广域域名。
...
- 从 `~/.zoidbergbot/dns/<domain>.db` 提供你选择的域名服务（示例：`zoidbergbot.internal.`）

从 Tailnet 连接的机器上验证：

```bash
dns-sd -B _zoidbergbot-gw._tcp zoidbergbot.internal.
dig @<TAILNET_IPV4> -p 53 _zoidbergbot-gw._tcp.zoidbergbot.internal PTR +short
```
...
客户端接受 Tailnet DNS 后，iOS 节点即可在你的发现域名中浏览 `_zoidbergbot-gw._tcp`，无需组播。
...
- 在 `~/.zoidbergbot/zoidbergbot.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway网关（或重启 macOS 菜单栏应用）。

## 广播方

只有 Gateway网关广播 `_zoidbergbot-gw._tcp`。

## 服务类型

- `_zoidbergbot-gw._tcp` — Gateway网关传输信标（供 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<路径>`（可选；可运行的 `zoidbergbot` 入口点的绝对路径）
...
- 浏览实例：
  ```bash
  dns-sd -B _zoidbergbot-gw._tcp local.
  ```
- 解析单个实例（替换 `<instance>`）：
  ```bash
  dns-sd -L "<instance>" _zoidbergbot-gw._tcp local.
  ```
...
iOS 节点使用 `NWBrowser` 来发现 `_zoidbergbot-gw._tcp`。
...
- `ZOIDBERGBOT_DISABLE_BONJOUR=1` 禁用广播（旧版：`ZOIDBERGBOT_DISABLE_BONJOUR`）。
- `~/.zoidbergbot/zoidbergbot.json` 中的 `gateway.bind` 控制 Gateway网关绑定模式。
- `ZOIDBERGBOT_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`ZOIDBERGBOT_SSH_PORT`）。
- `ZOIDBERGBOT_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`ZOIDBERGBOT_TAILNET_DNS`）。
- `ZOIDBERGBOT_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`ZOIDBERGBOT_CLI_PATH`）。
