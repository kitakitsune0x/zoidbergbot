...
read_when:
  ...
  - 调试上下文增长或压缩行为时
summary: ZoidbergBot 如何构建提示上下文以及报告 token 使用量和费用
title: Token 使用与费用
x-i18n:
...
ZoidbergBot 跟踪的是 **token**，而非字符。Token 因模型而异，但大多数 OpenAI 风格的模型对英文文本平均约 4 个字符对应 1 个 token。

## 系统提示的构建方式

ZoidbergBot 在每次运行时组装自己的系统提示。它包括：
...
- `/usage cost` → 显示来自 ZoidbergBot 会话日志的本地费用摘要。

其他界面：

- **TUI/Web TUI：** 支持 `/status` + `/usage`。
- **CLI：** `zoidbergbot status --usage` 和 `zoidbergbot channels list` 显示提供商配额窗口（非逐条响应费用）。
...
这些是 `input`、`output`、`cacheRead` 和 `cacheWrite` 的**每百万 token 美元价格**。如果缺少定价信息，ZoidbergBot 仅显示 token 数。OAuth token 永远不显示美元费用。

## 缓存 TTL 与裁剪影响

提供商的提示缓存仅在缓存 TTL 窗口内有效。ZoidbergBot 可以选择性地运行**缓存 TTL 裁剪**：在缓存 TTL 过期后裁剪会话，然后重置缓存窗口，使后续请求可以复用新缓存的上下文，而不是重新缓存完整历史。这可以在会话空闲超过 TTL 后降低缓存写入费用。
