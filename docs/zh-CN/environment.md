...
read_when:
  ...
  - 正在编写提供商认证或部署环境的文档
summary: ZoidbergBot 从哪里加载环境变量及其优先级顺序
title: 环境变量
x-i18n:
...
ZoidbergBot 从多个来源获取环境变量。规则是**永远不覆盖已有的值**。

## 优先级（从高到低）

1. **进程环境**（Gateway网关进程从父 shell/守护进程继承的变量）。
2. **当前工作目录下的 `.env`**（dotenv 默认行为；不覆盖已有值）。
3. **全局 `.env`**，位于 `~/.zoidbergbot/.env`（即 `$ZOIDBERGBOT_STATE_DIR/.env`；不覆盖已有值）。
4. **配置文件中的 `env` 块**，位于 `~/.zoidbergbot/zoidbergbot.json`（仅在变量缺失时应用）。
5. **可选的登录 shell 导入**（`env.shellEnv.enabled` 或 `ZOIDBERGBOT_LOAD_SHELL_ENV=1`），仅对缺失的预期键生效。
