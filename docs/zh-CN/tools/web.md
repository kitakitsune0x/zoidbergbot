...
3. 运行 `zoidbergbot configure --section web` 将密钥存储到配置中（推荐），或在环境中设置 `BRAVE_API_KEY`。
...
**推荐：**运行 `zoidbergbot configure --section web`。它会将密钥存储在
...
**环境变量替代方案：**在 Gateway网关环境中设置 `OPENROUTER_API_KEY` 或 `PERPLEXITY_API_KEY`。对于 Gateway网关安装，将其放入 `~/.zoidbergbot/.env`。

如果未设置基础 URL，ZoidbergBot 会根据 API 密钥来源选择默认值：
