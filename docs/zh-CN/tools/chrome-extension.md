...
read_when:
  ...
  - 你想了解浏览器接管的安全影响
summary: Chrome 扩展：让 ZoidbergBot 控制你现有的 Chrome 标签页
title: Chrome 扩展
x-i18n:
...
ZoidbergBot Chrome 扩展让智能体控制你**现有的 Chrome 标签页**（你日常使用的 Chrome 窗口），而不是启动一个单独的 ZoidbergBot 托管 Chrome 配置文件。
...
ZoidbergBot 随后通过常规的 `browser` 工具界面（选择正确的配置文件）控制附加的标签页。
...
扩展作为静态文件包含在 ZoidbergBot 发行版（npm 包）中。没有单独的"构建"步骤。

升级 ZoidbergBot 后：

- 重新运行 `zoidbergbot browser extension install` 以刷新 ZoidbergBot 状态目录下的已安装文件。
- Chrome → `chrome://extensions` → 点击扩展上的"重新加载"。

## 使用方法（无需额外配置）

ZoidbergBot 内置了一个名为 `chrome` 的浏览器配置文件，指向默认端口上的扩展中继。
...
调试：`zoidbergbot sandbox explain`
...
`zoidbergbot browser extension path` 打印包含扩展文件的**已安装**磁盘目录。

CLI 故意**不会**打印 `node_modules` 路径。请始终先运行 `zoidbergbot browser extension install`，将扩展复制到 ZoidbergBot 状态目录下的稳定位置。
...
- 扩展使用 Chrome 的调试器 API（`chrome.debugger`）。附加后，模型可以：
  ...
  - 读取页面内容
  - 访问该标签页已登录会话能访问的任何内容
- **这不像**专用的 ZoidbergBot 托管配置文件那样是隔离的。
