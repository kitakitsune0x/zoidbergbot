...
read_when:
  - 添加或修改智能体工具
  - 停用或更改 `zoidbergbot-*` Skills
summary: ZoidbergBot 的智能体工具集（browser、canvas、nodes、message、cron），替代旧版 `zoidbergbot-*` Skills
title: 工具
x-i18n:
...
ZoidbergBot 提供**一等智能体工具**，涵盖 browser、canvas、nodes 和 cron。
这些工具替代了旧的 `zoidbergbot-*` Skills：工具是类型化的，无需 shell 调用，
智能体应直接依赖这些工具。

## 禁用工具

你可以通过 `zoidbergbot.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具
...
- 匹配不区分大小写。
- 支持 `*` 通配符（`"*"` 表示所有工具）。
- 如果 `tools.allow` 仅引用了未知或未加载的插件工具名称，ZoidbergBot 会记录警告并忽略允许列表，以确保核心工具保持可用。
...
- `group:nodes`：`nodes`
- `group:openclaw`：所有内置 ZoidbergBot 工具（不包括提供商插件）
...
- `host=node` 可以指向 macOS 伴侣应用或无头节点主机（`zoidbergbot node run`）。
...
- 需要 Brave API 密钥（推荐：`zoidbergbot configure --section web`，或设置 `BRAVE_API_KEY`）。
...
- 快速测试：`zoidbergbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`zoidbergbot gateway` 原地重启）
