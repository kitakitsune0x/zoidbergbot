...
ZoidbergBot 控制一个**专用 Chrome 配置文件**（名为 `zoidbergbot`，橙色调界面）。它与你的日常浏览器配置文件是分开的。
...
zoidbergbot browser open https://x.com
```

如果你有多个配置文件，传入 `--browser-profile <name>`（默认值为 `zoidbergbot`）。
...
```bash
zoidbergbot browser open https://x.com --browser-profile zoidbergbot --target host
```
