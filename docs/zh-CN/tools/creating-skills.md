...
ZoidbergBot 的设计易于扩展。"Skills"是为你的助手添加新功能的主要方式。
...
Skills 存放在你的工作区中，通常位于 `~/.zoidbergbot/workspace/skills/`。为你的 Skills 创建一个新文件夹：

```bash
mkdir -p ~/.zoidbergbot/workspace/skills/hello-world
```
...
让你的智能体"刷新 Skills"或重启 Gateway网关。ZoidbergBot 将发现新目录并索引 `SKILL.md`。

## 最佳实践

- **保持简洁**：指示模型应该做*什么*，而不是如何成为一个 AI。
- **安全第一**：如果你的 Skills 使用 `bash`，请确保提示不会允许来自不受信任用户输入的任意命令注入。
- **本地测试**：使用 `zoidbergbot agent --message "use my new skill"` 进行测试。
