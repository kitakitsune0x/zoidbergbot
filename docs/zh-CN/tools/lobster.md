---
description: Typed workflow runtime for ZoidbergBot — composable pipelines with approval gates.
read_when:
  - 你需要具有显式审批的确定性多步骤工作流
  - 你需要恢复工作流而无需重新运行之前的步骤
summary: ZoidbergBot 的类型化工作流运行时，支持可恢复的审批门控。
title: Lobster
x-i18n:
...
Lobster 是一个工作流外壳，让 ZoidbergBot 能够将多步骤工具序列作为单个确定性操作运行，并带有显式审批检查点。
...
- **一次调用代替多次**：ZoidbergBot 运行一次 Lobster 工具调用即可获得结构化结果。
...
ZoidbergBot 以**工具模式**启动本地 `lobster` CLI，并从 stdout 解析 JSON 信封。
...
gog.gmail.search --query 'newer_than:1d' \
  | zoidbergbot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
zoidbergbot.invoke --tool llm-task --action json --args-json '{
...
Lobster 可以运行包含 `name`、`args`、`steps`、`env`、`condition` 和 `approval` 字段的 YAML/JSON 工作流文件。在 ZoidbergBot 工具调用中，将 `pipeline` 设置为文件路径。
...
在运行 ZoidbergBot Gateway网关的**同一主机**上安装 Lobster CLI（参见 [Lobster 仓库](https://github.com/zoidbergbot/lobster)），并确保 `lobster` 在 `PATH` 中。
...
插件工具（如 `lobster`），ZoidbergBot 会保持核心工具启用。要限制核心
...
```
用户："检查我的邮件并起草回复"
→ zoidbergbot 调用 gmail.list
→ LLM 进行摘要
→ 用户："给 #2 和 #5 起草回复"
→ LLM 起草
→ 用户："发送 #2"
→ zoidbergbot 调用 gmail.send
（每天重复，不记得之前分类了什么）
```
...
- **仅限本地子进程** — 插件本身不发起网络调用。
- **无密钥** — Lobster 不管理 OAuth；它调用处理 OAuth 的 ZoidbergBot 工具。
