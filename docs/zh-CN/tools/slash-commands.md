...
- `commands.config`（默认 `false`）启用 `/config`（读写 `zoidbergbot.json`）。
...
- 要查看完整的提供商用量明细，请使用 `zoidbergbot status --usage`。
- `/allowlist add|remove` 需要 `commands.config=true` 并遵循渠道的 `configWrites` 设置。
- `/usage` 控制每次响应的用量页脚；`/usage cost` 从 ZoidbergBot 会话日志中打印本地费用摘要。
...
- 覆盖会立即应用于新的配置读取，但**不会**写入 `zoidbergbot.json`。
- 使用 `/debug reset` 清除所有覆盖并恢复到磁盘上的配置。

## 配置更新

`/config` 写入你的磁盘配置（`zoidbergbot.json`）。仅所有者可用。默认禁用；通过 `commands.config: true` 启用。
...
/config set messages.responsePrefix="[zoidbergbot]"
...
- **Slack：**仍然支持 `channels.slack.slashCommand` 用于单个 `/zoidbergbot` 风格的命令。如果你启用了 `commands.native`，则必须为每个内置命令创建一个 Slack 斜杠命令（名称与 `/help` 相同）。Slack 的命令参数菜单以临时 Block Kit 按钮形式呈现。
