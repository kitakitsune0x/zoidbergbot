...
read_when:
  - 添加或修改智能体 CLI 入口点
summary: 直接通过 `zoidbergbot agent` CLI 运行（支持可选的消息投递）
title: 智能体发送
x-i18n:
...
# `zoidbergbot agent`（直接运行智能体）

`zoidbergbot agent` 无需入站聊天消息即可运行单次智能体轮次。
...
- 输出：
  ...
  - `--json`：打印结构化载荷 + 元数据
- 可选通过 `--deliver` + `--channel` 将回复投递回渠道（目标格式与 `zoidbergbot message --target` 一致）。
...
```bash
zoidbergbot agent --to +15555550123 --message "status update"
zoidbergbot agent --agent ops --message "Summarize logs"
zoidbergbot agent --session-id 1234 --message "Summarize inbox" --thinking medium
zoidbergbot agent --to +15555550123 --message "Trace logs" --verbose on --json
zoidbergbot agent --to +15555550123 --message "Summon reply" --deliver
zoidbergbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
```
