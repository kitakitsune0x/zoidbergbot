...
ZoidbergBot 可以使用 **Firecrawl** 作为 `web_fetch` 的备用提取器。它是一个托管的内容提取服务，支持反爬虫绕过和缓存，有助于处理 JS 密集型网站或阻止普通 HTTP 请求的页面。
...
ZoidbergBot 对 Firecrawl 请求始终使用 `proxy: "auto"` 加上 `storeInCache: true`。
