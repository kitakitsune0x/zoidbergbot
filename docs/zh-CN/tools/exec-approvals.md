...
- **Gateway网关主机** → Gateway网关机器上的 `zoidbergbot` 进程
...
如果某个节点尚未广播执行审批功能，请直接编辑其本地的 `~/.zoidbergbot/exec-approvals.json` 文件。

CLI：`zoidbergbot approvals` 支持 Gateway网关或节点编辑（参见 [审批 CLI](/cli/approvals)）。
