...
本指南总结了在 ZoidbergBot 中进行 Pi 集成开发的合理工作流。
...
- 以开发模式运行 Gateway网关：
  - `pnpm gateway:dev`
- 直接触发智能体：
  - `pnpm zoidbergbot agent --message "Hello" --thinking low`
- 使用 TUI 进行交互式调试：
  - `pnpm tui`
...
状态存储在 ZoidbergBot 状态目录下。默认为 `~/.zoidbergbot`。如果设置了 `ZOIDBERGBOT_STATE_DIR`，则使用该目录。

要重置所有内容：

- `zoidbergbot.json` 用于配置
...
- https://docs.zoidbergbot.ai/testing
- https://docs.zoidbergbot.ai/start/getting-started
