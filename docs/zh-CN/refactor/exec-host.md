...
- **询问默认值：**`on-miss`。
- **审批存储：**`~/.zoidbergbot/exec-approvals.json`（JSON，不迁移旧版）。
- **运行器：**无头系统服务；UI 应用托管 Unix socket 用于审批。
- **节点身份：**使用现有的 `nodeId`。
- **Socket 认证：**Unix socket + token（跨平台）；后续需要时再拆分。
- **节点主机状态：**`~/.zoidbergbot/node.json`（节点 ID + 配对 token）。
...
路径：`~/.zoidbergbot/exec-approvals.json`
...
- Unix socket 位于 `~/.zoidbergbot/exec-approvals.sock`（0600）。
