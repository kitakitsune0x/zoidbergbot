...
ZoidbergBot 拥有三个 Vitest 测试套件（单元/集成、端到端、实时）和一小组 Docker 运行器。
...
- 文件：`src/**/*.live.test.ts`
- 默认：通过 `pnpm test:live` **启用**（设置 `ZOIDBERGBOT_LIVE_TEST=1`）
- 范围：
...
- 预期：
  ...
  - Anthropic 密钥轮换：设置 `ZOIDBERGBOT_LIVE_ANTHROPIC_KEYS="sk-...,sk-..."`（或 `ZOIDBERGBOT_LIVE_ANTHROPIC_KEY=sk-...`）或多个 `ANTHROPIC_API_KEY*` 变量；测试会在遇到速率限制时重试
...
- 如何启用：
  - `pnpm test:live`（或直接调用 Vitest 时设置 `ZOIDBERGBOT_LIVE_TEST=1`）
- 设置 `ZOIDBERGBOT_LIVE_MODELS=modern`（或 `all`，modern 的别名）来实际运行此套件；否则会跳过以保持 `pnpm test:live` 聚焦于 Gateway网关冒烟测试
- 如何选择模型：
  - `ZOIDBERGBOT_LIVE_MODELS=modern` 运行现代允许列表（Opus/Sonnet/Haiku 4.5、GPT-5.x + Codex、Gemini 3、GLM 4.7、MiniMax M2.1、Grok 4）
  - `ZOIDBERGBOT_LIVE_MODELS=all` 是现代允许列表的别名
  - 或 `ZOIDBERGBOT_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,..."`（逗号分隔的允许列表）
- 如何选择提供商：
  - `ZOIDBERGBOT_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"`（逗号分隔的允许列表）
- 密钥来源：
  - 默认：profile 存储和环境变量回退
  - 设置 `ZOIDBERGBOT_LIVE_REQUIRE_PROFILE_KEYS=1` 以强制仅使用 **profile 存储**
- 为何存在：
...
- 如何选择模型：
  ...
  - `ZOIDBERGBOT_LIVE_GATEWAY_MODELS=all` 是现代允许列表的别名
  - 或设置 `ZOIDBERGBOT_LIVE_GATEWAY_MODELS="provider/model"`（或逗号分隔列表）来缩小范围
- 如何选择提供商（避免"OpenRouter 全部"）：
  - `ZOIDBERGBOT_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"`（逗号分隔的允许列表）
- 工具 + 图像探测在此实时测试中始终开启：
...
- 令牌来源（选一个）：
  ...
  - 原始令牌：`ZOIDBERGBOT_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- 模型覆盖（可选）：
...
- 覆盖（可选）：
  ...
  - `ZOIDBERGBOT_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 发送真实图像附件（路径注入到提示中）。
  - `ZOIDBERGBOT_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` 将图像文件路径作为 CLI 参数传递而非提示注入。
  - `ZOIDBERGBOT_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`（或 `"list"`）控制设置 `IMAGE_ARG` 时图像参数的传递方式。
  - `ZOIDBERGBOT_LIVE_CLI_BACKEND_RESUME_PROBE=1` 发送第二轮并验证恢复流程。
- `ZOIDBERGBOT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` 保持 Claude Code CLI MCP 配置启用（默认使用临时空文件禁用 MCP 配置）。
...
- Google 专项（Gemini API 密钥 + Antigravity）：
  - Gemini（API 密钥）：`ZOIDBERGBOT_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API 与 Gemini CLI：
  - API：ZoidbergBot 通过 HTTP 调用 Google 的托管 Gemini API（API 密钥/profile 认证）；这是大多数用户所说的"Gemini"。
  - CLI：ZoidbergBot 调用本地 `gemini` 二进制文件；它有自己的认证，行为可能不同（流式传输/工具支持/版本差异）。
...
在 `ZOIDBERGBOT_LIVE_GATEWAY_MODELS` 中至少包含一个支持图像的模型（Claude/Gemini/OpenAI 视觉能力变体等）以测试图像探测。

### 聚合器/替代网关

如果你启用了密钥，我们还支持通过以下方式测试：

- OpenRouter：`openrouter/...`（数百个模型；使用 `zoidbergbot models scan` 查找支持工具+图像的候选模型）
...
- 如果实时测试提示"无凭证"，按照调试 `zoidbergbot models list`/模型选择的方式来调试。

- Profile 存储：`~/.zoidbergbot/credentials/`（首选；测试中"profile 密钥"的含义）
- 配置：`~/.zoidbergbot/zoidbergbot.json`（或 `ZOIDBERGBOT_CONFIG_PATH`）
...
- `ZOIDBERGBOT_CONFIG_DIR=...`（默认：`~/.zoidbergbot`）挂载到 `/home/node/.openclaw`
- `ZOIDBERGBOT_WORKSPACE_DIR=...`（默认：`~/.zoidbergbot/workspace`）挂载到 `/home/node/.zoidbergbot/workspace`
- `ZOIDBERGBOT_PROFILE_FILE=...`（默认：`~/.profile`）挂载到 `/home/node/.profile` 并在运行测试前加载
- `ZOIDBERGBOT_LIVE_GATEWAY_MODELS=...` / `ZOIDBERGBOT_LIVE_MODELS=...` 缩小运行范围
- `ZOIDBERGBOT_LIVE_REQUIRE_PROFILE_KEYS=1` 确保凭证来自 profile 存储（而非环境变量）
