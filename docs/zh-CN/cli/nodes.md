...
read_when:
  - 正在管理配对节点（摄像头、屏幕、画布）
  - 需要批准请求或调用节点命令
summary: 管理配对节点（列表/状态/批准/调用，摄像头/画布/屏幕）的 `zoidbergbot nodes` CLI 参考
title: nodes
x-i18n:
...
zoidbergbot nodes list --connected
zoidbergbot nodes list --last-connected 24h
zoidbergbot nodes pending
zoidbergbot nodes approve <requestId>
zoidbergbot nodes status
zoidbergbot nodes status --connected
zoidbergbot nodes status --last-connected 24h
```
...
```bash
zoidbergbot nodes invoke --node <id|name|ip> --command <command> --params <json>
zoidbergbot nodes run --node <id|name|ip> <command...>
zoidbergbot nodes run --raw "git status"
zoidbergbot nodes run --agent main --node <id|name|ip> --raw "git status"
```
