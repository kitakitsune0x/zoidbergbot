...
read_when:
  - 添加或修改 CLI 命令或选项
  - 为新的命令界面编写文档
summary: ZoidbergBot `zoidbergbot` 命令、子命令和选项的 CLI 参考
title: CLI 参考
x-i18n:
...
- `--dev`：将状态隔离到 `~/.zoidbergbot-dev` 并偏移默认端口。
- `--profile <name>`：将状态隔离到 `~/.zoidbergbot-<name>`。
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`zoidbergbot update` 的简写（仅限源码安装）。
- `-V`、`--version`、`-v`：打印版本并退出。
...
ZoidbergBot 使用龙虾色配色方案作为 CLI 输出。
...
注意：插件可以添加额外的顶层命令（例如 `zoidbergbot voicecall`）。

## 安全

- `zoidbergbot security audit` — 审计配置和本地状态中常见的安全隐患。
- `zoidbergbot security audit --deep` — 尽力进行实时 Gateway网关探测。
- `zoidbergbot security audit --fix` — 收紧安全默认设置并修改状态/配置文件权限。

## 插件

管理扩展及其配置：

- `zoidbergbot plugins list` — 发现插件（使用 `--json` 获取机器可读输出）。
- `zoidbergbot plugins info <id>` — 显示插件详情。
- `zoidbergbot plugins install <path|.tgz|npm-spec>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `zoidbergbot plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `zoidbergbot plugins doctor` — 报告插件加载错误。
...
- `--workspace <dir>`：智能体工作区路径（默认 `~/.zoidbergbot/workspace`）。
- `--wizard`：运行新手引导向导。
...
非交互式配置辅助工具（get/set/unset）。不带子命令运行 `zoidbergbot config` 将启动向导。
...
- `channels status`：检查 Gateway网关可达性和渠道健康状态（`--probe` 运行额外检查；使用 `openclaw health` 或 `zoidbergbot status --deep` 进行 Gateway网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印警告并提供修复建议（然后引导你使用 `zoidbergbot doctor`）。
...
```bash
zoidbergbot channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
zoidbergbot channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
zoidbergbot channels remove --channel discord --account work --delete
zoidbergbot channels status --probe
...
- `zoidbergbot message send --target +15555550123 --message "Hi"`
- `zoidbergbot message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
当 OAuth/API 凭证可用时，ZoidbergBot 可以展示提供商的用量/配额信息。

展示位置：

- `/status`（可用时添加简短的提供商用量行）
- `zoidbergbot status --usage`（打印完整的提供商用量明细）
...
- `gateway status` 还会在检测到旧版或额外的 Gateway网关服务时展示（`--deep` 添加系统级扫描）。以配置文件命名的 ZoidbergBot 服务被视为正式服务，不会被标记为"额外"。
...
zoidbergbot logs --limit 200
zoidbergbot logs --plain
zoidbergbot logs --json
zoidbergbot logs --no-color
```
...
`zoidbergbot models` 是 `models status` 的别名。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`zoidbergbot node`](/cli/node)。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`zoidbergbot browser`](/cli/browser) 和[浏览器工具](/tools/browser)。
