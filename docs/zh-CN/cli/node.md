...
summary: "`zoidbergbot node`（无头节点主机）的 CLI 参考"
title: node
x-i18n:
...
```bash
zoidbergbot node install --host <gateway-host> --port 18789
```
...
```bash
zoidbergbot node status
zoidbergbot node stop
zoidbergbot node restart
zoidbergbot node uninstall
```

使用 `zoidbergbot node run` 进行前台节点主机运行（无服务）。
...
- `zoidbergbot approvals --node <id|name|ip>`（从 Gateway网关编辑）
