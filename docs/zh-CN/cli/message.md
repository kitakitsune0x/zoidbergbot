...
read_when:
  - 添加或修改消息 CLI 操作
  - 更改出站渠道行为
summary: "`zoidbergbot message`（发送 + 渠道操作）的 CLI 参考"
title: message
x-i18n:
...
```
zoidbergbot message <subcommand> [flags]
```
...
- 缓存未命中时，如果提供商支持，ZoidbergBot 将尝试实时目录查找。
...
```
zoidbergbot message send --channel discord \
...
```
zoidbergbot message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
zoidbergbot message poll --channel msteams \
...
```
zoidbergbot message react --channel slack \
...
```
zoidbergbot message react --channel signal \
...
```
zoidbergbot message send --channel telegram --target @mychat --message "Choose:" \
