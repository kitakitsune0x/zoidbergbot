...
运行与 ZoidbergBot Gateway网关通信的 ACP（Agent Client Protocol）桥接。
...
# 在第一个提示之前重置会话密钥
zoidbergbot acp --session agent:main:main --reset-session
```
...
```bash
zoidbergbot acp client

# 将启动的桥接指向远程 Gateway网关
zoidbergbot acp client --server-args --url wss://gateway-host:18789 --token <token>

# 覆盖服务器命令（默认：openclaw）
zoidbergbot acp client --server "node" --server-args zoidbergbot.mjs acp --url ws://127.0.0.1:19001
```

## 如何使用

当 IDE（或其他客户端）使用 Agent Client Protocol 并且你希望它驱动 ZoidbergBot Gateway网关会话时，请使用 ACP。

1. 确保 Gateway网关正在运行（本地或远程）。
2. 配置 Gateway网关目标（通过配置文件或标志）。
3. 将你的 IDE 配置为通过 stdio 运行 `zoidbergbot acp`。
...
在 Zed 中，打开 Agent 面板并选择 "ZoidbergBot ACP" 来开始一个对话线程。
...
- `--cwd <dir>`：ACP 会话的工作目录。
- `--server <command>`：ACP 服务器命令（默认：`zoidbergbot`）。
