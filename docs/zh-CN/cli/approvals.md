...
summary: "`zoidbergbot approvals` 的 CLI 参考（用于 Gateway网关或节点主机的执行审批）"
title: approvals
x-i18n:
...
- `--node` 使用与 `zoidbergbot nodes` 相同的解析器（id、名称、ip 或 id 前缀）。
- `--agent` 默认为 `"*"`，即适用于所有智能体。
- 节点主机必须公布 `system.execApprovals.get/set`（macOS 应用或无头节点主机）。
- 审批文件按主机存储在 `~/.zoidbergbot/exec-approvals.json`。
