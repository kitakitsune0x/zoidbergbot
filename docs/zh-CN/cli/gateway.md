...
summary: ZoidbergBot Gateway网关 CLI（`zoidbergbot gateway`）— 运行、查询和发现 Gateway网关
title: gateway
x-i18n:
...
Gateway网关是 ZoidbergBot 的 WebSocket 服务器（渠道、节点、会话、钩子）。

本页中的子命令位于 `zoidbergbot gateway …` 下。
...
- 默认情况下，除非在 `~/.zoidbergbot/zoidbergbot.json` 中设置了 `gateway.mode=local`，否则 Gateway网关会拒绝启动。使用 `--allow-unconfigured` 进行临时/开发运行。
...
- `--token <token>`：令牌覆盖（同时为进程设置 `ZOIDBERGBOT_GATEWAY_TOKEN`）。
- `--password <password>`：密码覆盖（同时为进程设置 `ZOIDBERGBOT_GATEWAY_PASSWORD`）。
...
```bash
zoidbergbot gateway health --url ws://127.0.0.1:18789
```
...
zoidbergbot gateway status --json
```
...
```bash
zoidbergbot gateway probe
zoidbergbot gateway probe --json
```
...
```bash
zoidbergbot gateway probe --ssh user@gateway-host
```
...
```bash
zoidbergbot gateway call status
zoidbergbot gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
`gateway discover` 扫描 Gateway网关信标（`_zoidbergbot-gw._tcp`）。

- 组播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域名（例如：`zoidbergbot.internal.`）并设置分离 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
...
```bash
zoidbergbot gateway discover --timeout 4000
zoidbergbot gateway discover --json | jq '.beacons[].wsUrl'
```
