...
summary: "`zoidbergbot configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令运行 `zoidbergbot config` 会打开相同的向导。使用
`zoidbergbot config get|set|unset` 进行非交互式编辑。
...
```bash
zoidbergbot configure
zoidbergbot configure --section models --section channels
```
