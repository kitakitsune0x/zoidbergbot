...
read_when:
  - 你想安全地更新源码检出
  - 你需要了解 `--update` 简写行为
summary: "`zoidbergbot update`（安全的源码更新 + Gateway网关自动重启）的 CLI 参考"
title: update
x-i18n:
...
安全更新 ZoidbergBot 并在 stable/beta/dev 渠道之间切换。
...
```bash
zoidbergbot update status
zoidbergbot update status --json
zoidbergbot update status --timeout 10
```
...
当你显式切换渠道（`--channel ...`）时，ZoidbergBot 也会保持安装方式一致：

- `dev` → 确保存在 git 检出（默认：`~/zoidbergbot`，可通过 `ZOIDBERGBOT_GIT_DIR` 覆盖），更新它，并从该检出安装全局 CLI。
...
7. 构建项目 + 构建控制台 UI。
8. 运行 `zoidbergbot doctor` 作为最终的"安全更新"检查。
9. 将插件同步到活跃渠道（dev 使用内置扩展；stable/beta 使用 npm）并更新通过 npm 安装的插件。

## `--update` 简写

`zoidbergbot --update` 会重写为 `zoidbergbot update`（便于在 shell 和启动脚本中使用）。

## 另请参阅

- `zoidbergbot doctor`（在 git 检出上会提供先运行更新的选项）
