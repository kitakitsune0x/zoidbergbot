...
summary: 由 ZoidbergBot 驱动的社区项目和集成展示
title: 项目展示
x-i18n:
...
OpenCode 完成修改 → 提交 PR → ZoidbergBot 审查差异并在 Telegram 中回复"小建议"以及明确的合并意见（包括需要优先应用的关键修复）。

  <img src="/assets/showcase/pr-review-telegram.jpg" alt="ZoidbergBot PR 审查反馈通过 Telegram 发送" />
</Card>
...
桌面应用，用于跨 Agents、Claude、Codex 和 ZoidbergBot 管理 Skills/命令。
...
上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 ZoidbergBot 实例。
</Card>
...
集成智能体工作流（Claude Code、ZoidbergBot）的 Linear CLI。从终端管理 issue、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制方式，然后 ZoidbergBot 接管并管理室内空气质量。

  <img src="/assets/showcase/winix-air-purifier.jpg" alt="通过 ZoidbergBot 控制 Winix 空气净化器" />
</Card>

<Card title="美丽天空相机抓拍" icon="camera" href="https://x.com/signalgaining/status/2010523120604746151">
  **@signalgaining** • `automation` `camera` `skill` `images`

由屋顶摄像头触发：当天空看起来很美时，让 ZoidbergBot 拍一张天空照片——它设计了一个 Skills 并完成了拍摄。

  <img src="/assets/showcase/roof-camera-sky.jpg" alt="ZoidbergBot 捕获的屋顶摄像头天空快照" />
</Card>

<Card title="可视化早间简报场景" icon="robot" href="https://x.com/buddyhadry/status/2010005331925954739">
  **@buddyhadry** • `automation` `briefing` `images` `telegram`

定时提示每天早上通过 ZoidbergBot 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/名言）。
</Card>
...
ZoidbergBot 连接到 Jira，然后即时生成了一个新 Skills（在 ClawHub 上还不存在时）。
</Card>
...
自动化 Todoist 任务，并让 ZoidbergBot 直接在 Telegram 聊天中生成 Skills。
</Card>
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  通过 ZoidbergBot 实现的中文学习引擎，具备发音反馈和学习流程。
...
<Card title="Clawdia 电话桥接" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  ...
  Vapi 语音助手 ↔ ZoidbergBot HTTP 桥接。与你的智能体进行近实时电话通话。
</Card>
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/zoidbergbot-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  
  在 Home Assistant OS 上运行 ZoidbergBot Gateway网关，支持 SSH 隧道和持久化状态。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  以 Nix 为基础的家居自动化，ZoidbergBot 作为交互界面，并配有精美的 Grafana 仪表盘。
...
<Card title="StarSwap 交易市场" icon="star" href="https://star-swap.com/">
  **社区** • `marketplace` `astronomy` `webapp`
  ...
  完整的天文设备交易市场。基于 ZoidbergBot 生态系统构建。
</Card>
