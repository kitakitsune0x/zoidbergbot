...
- 旧版 `node.pair.*` API（CLI：`zoidbergbot nodes pending/approve`）是一个独立的 Gateway网关所拥有的配对存储。WebSocket 节点仍然需要设备配对。
