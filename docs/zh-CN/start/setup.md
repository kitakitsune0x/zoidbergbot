...
summary: 设置指南：在保持更新的同时维护你的个性化 ZoidbergBot 配置
title: 设置
x-i18n:
...
- **个性化配置存放在仓库之外：** `~/.zoidbergbot/workspace`（工作区）+ `~/.zoidbergbot/zoidbergbot.json`（配置）。
...
- **配置：** `~/.zoidbergbot/zoidbergbot.json`（JSON/JSON5 风格）
- **工作区：** `~/.zoidbergbot/workspace`（Skills、提示词、记忆；建议设为私有 git 仓库）
...
如果你还没有全局安装，请通过 `pnpm zoidbergbot setup` 运行。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **ZoidbergBot.app**（菜单栏）。
...
- 运行 `openclaw setup`，然后 `zoidbergbot channels login`，接着手动启动 Gateway网关（`zoidbergbot gateway`）。
...
### 2) 将 macOS 应用指向你运行的 Gateway网关在 **ZoidbergBot.app** 中：
...
- **模型认证配置文件**：`~/.zoidbergbot/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.zoidbergbot/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的配置）

- 将 `~/.zoidbergbot/workspace` 和 `~/.zoidbergbot/` 视为"你的东西"；不要将个人提示词/配置放入 `zoidbergbot` 仓库中。
