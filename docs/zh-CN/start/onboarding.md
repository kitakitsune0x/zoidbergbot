...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: ZoidbergBot 首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭据写入 `~/.zoidbergbot/credentials/oauth.json`
...
应用可以通过 npm/pnpm 安装全局 `zoidbergbot` CLI，使终端工作流和 launchd 任务开箱即用。
...
在智能体首次运行时，ZoidbergBot 会引导创建工作区（默认 `~/.zoidbergbot/workspace`）：
