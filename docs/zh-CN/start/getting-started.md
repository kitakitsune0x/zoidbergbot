...
- **推荐：** Brave Search API 密钥用于网络搜索。最简单的方式：
  `zoidbergbot configure --section web` （存储 `tools.web.search.apiKey`）。
  参见 [网络工具](/tools/web)。
...
```bash
curl -fsSL https://zoidbergbot.bot/install.sh | bash
```
...
- OAuth 凭据（旧版导入）： `~/.zoidbergbot/credentials/oauth.json`
...
如果你尚未进行全局安装，请通过以下方式运行新手引导流程 `pnpm zoidbergbot ...` （从仓库中）。
`pnpm build` 也会打包 A2UI 资源；如果你只需要运行该步骤，请使用 `pnpm canvas:a2ui:bundle`。

Gateway网关（从此仓库）：

```bash
node zoidbergbot.mjs gateway --port 18789 --verbose
```
...
提示： `zoidbergbot status --all` 是最佳的可粘贴只读调试报告。
健康探针： `openclaw health` （或 `zoidbergbot status --deep`）向运行中的 Gateway网关请求健康快照。
