...
钩子提供了一个可扩展的事件驱动系统，用于在响应智能体命令和事件时自动执行操作。钩子从目录中自动发现，并可通过 CLI 命令管理，类似于 ZoidbergBot 中 Skills 的工作方式。
...
- **Webhook**：外部 HTTP webhook，允许其他系统在 ZoidbergBot 中触发工作。参见 [Webhook 钩子](/automation/webhook) 或使用 `zoidbergbot webhooks` 获取 Gmail 辅助命令。
...
- 记录所有命令用于审计
- 在智能体生命周期事件上触发自定义自动化
- 扩展 ZoidbergBot 的行为而无需修改核心代码

## 快速开始

### 内置钩子

ZoidbergBot 附带四个自动发现的内置钩子：

- **💾 session-memory**：在你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.zoidbergbot/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.zoidbergbot/logs/commands.log`
...
```bash
zoidbergbot hooks list
```

启用钩子：

```bash
zoidbergbot hooks enable session-memory
```

检查钩子状态：

```bash
zoidbergbot hooks check
```

获取详细信息：

```bash
zoidbergbot hooks info session-memory
```
...
2. **托管钩子**：`~/.zoidbergbot/hooks/`（用户安装，跨工作区共享）
3. **内置钩子**：`<openclaw>/dist/hooks/bundled/`（随 ZoidbergBot 附带）
...
钩子包是标准的 npm 包，通过 `package.json` 中的 `zoidbergbot.hooks` 导出一个或多个钩子。使用以下命令安装：

```bash
zoidbergbot hooks install <path-or-spec>
```
...
钩子包可以附带依赖；它们将安装到 `~/.zoidbergbot/hooks/<id>` 下。
...
homepage: https://docs.zoidbergbot.ai/hooks#my-hook
metadata:
  { "zoidbergbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
这些钩子不是事件流监听器；它们允许插件在 ZoidbergBot 持久化工具结果之前同步调整工具结果。
...
- **托管钩子**（`~/.zoidbergbot/hooks/`）：跨工作区共享

### 2. 创建目录结构

```bash
mkdir -p ~/.zoidbergbot/hooks/my-hook
cd ~/.zoidbergbot/hooks/my-hook
```
...
metadata: { "zoidbergbot": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# 启用它
zoidbergbot hooks enable my-hook
...
# 仅显示符合条件的钩子
zoidbergbot hooks list --eligible

# 详细输出（显示缺失的要求）
zoidbergbot hooks list --verbose

# JSON 输出
zoidbergbot hooks list --json
```
...
# JSON 输出
zoidbergbot hooks info session-memory --json
```
...
# 禁用钩子
zoidbergbot hooks disable command-logger
```
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.zoidbergbot/workspace`）
...
**输出**：`~/.zoidbergbot/logs/commands.log`
...
```bash
# 查看最近的命令
tail -n 20 ~/.zoidbergbot/logs/commands.log

# 用 jq 美化输出
cat ~/.zoidbergbot/logs/commands.log | jq .

# 按操作过滤
grep '"action":"new"' ~/.zoidbergbot/logs/commands.log | jq .
```

**启用**：

```bash
zoidbergbot hooks enable command-logger
```
...
```bash
zoidbergbot hooks enable soul-evil
```
...
```bash
zoidbergbot hooks enable boot-md
```
...
```yaml
metadata: { "zoidbergbot": { "events": ["command:new"] } } # 具体
```

而不是：

```yaml
metadata: { "zoidbergbot": { "events": ["command"] } } # 通用 - 开销更大
```
...
```bash
zoidbergbot hooks info my-hook
```
...
# 其他平台
tail -f ~/.zoidbergbot/gateway.log
```
...
1. 检查目录结构：
   ...
   ```bash
   ls -la ~/.zoidbergbot/hooks/my-hook/
...
2. 验证 HOOK.md 格式：
   ...
   ```bash
   cat ~/.zoidbergbot/hooks/my-hook/HOOK.md
...
1. 创建钩子目录：
   ...
   ```bash
   mkdir -p ~/.zoidbergbot/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.zoidbergbot/hooks/my-hook/handler.ts
   ```
...
- [内置钩子 README](https://github.com/zoidbergbot/zoidbergbot/tree/main/src/hooks/bundled)
