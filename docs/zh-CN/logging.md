...
你可以在 `~/.zoidbergbot/zoidbergbot.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/zoidbergbot.log"
  }
}
```
...
```bash
zoidbergbot channels logs --channel whatsapp
```
...
所有日志配置位于 `~/.zoidbergbot/zoidbergbot.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/zoidbergbot/zoidbergbot-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- ZoidbergBot 目前通过 **OTLP/HTTP (protobuf)** 导出。
...
```
ZOIDBERGBOT_DIAGNOSTICS=telegram.http,telegram.payload
```
...
```json
{
  ...
  "diagnostics": {
    ...
    "otel": {
      ...
      "serviceName": "zoidbergbot-gateway",
      "traces": true,
      "metrics": true,
      "logs": true,
      "sampleRate": 0.2,
...
- 你也可以使用 `zoidbergbot plugins enable diagnostics-otel` 来启用插件。
...
- `zoidbergbot.tokens`（计数器，属性：`zoidbergbot.token`、`zoidbergbot.channel`、`zoidbergbot.provider`、`zoidbergbot.model`）
- `zoidbergbot.cost.usd`（计数器，属性：`zoidbergbot.channel`、`zoidbergbot.provider`、`zoidbergbot.model`）
- `zoidbergbot.run.duration_ms`（直方图，属性：`zoidbergbot.channel`、`zoidbergbot.provider`、`zoidbergbot.model`）
- `zoidbergbot.context.tokens`（直方图，属性：`zoidbergbot.context`、`zoidbergbot.channel`、`zoidbergbot.provider`、`zoidbergbot.model`）

消息流：

- `zoidbergbot.webhook.received`（计数器，属性：`zoidbergbot.channel`、`zoidbergbot.webhook`）
- `zoidbergbot.webhook.error`（计数器，属性：`zoidbergbot.channel`、`zoidbergbot.webhook`）
- `zoidbergbot.webhook.duration_ms`（直方图，属性：`zoidbergbot.channel`、`zoidbergbot.webhook`）
- `zoidbergbot.message.queued`（计数器，属性：`zoidbergbot.channel`、`zoidbergbot.source`）
- `zoidbergbot.message.processed`（计数器，属性：`zoidbergbot.channel`、`zoidbergbot.outcome`）
- `zoidbergbot.message.duration_ms`（直方图，属性：`zoidbergbot.channel`、`zoidbergbot.outcome`）

队列 + 会话：

- `zoidbergbot.queue.lane.enqueue`（计数器，属性：`zoidbergbot.lane`）
- `zoidbergbot.queue.lane.dequeue`（计数器，属性：`zoidbergbot.lane`）
- `zoidbergbot.queue.depth`（直方图，属性：`zoidbergbot.lane` 或 `zoidbergbot.channel=heartbeat`）
- `zoidbergbot.queue.wait_ms`（直方图，属性：`zoidbergbot.lane`）
- `zoidbergbot.session.state`（计数器，属性：`zoidbergbot.state`、`zoidbergbot.reason`）
- `zoidbergbot.session.stuck`（计数器，属性：`zoidbergbot.state`）
- `zoidbergbot.session.stuck_age_ms`（直方图，属性：`zoidbergbot.state`）
- `zoidbergbot.run.attempt`（计数器，属性：`zoidbergbot.attempt`）

### 导出的 span（名称 + 关键属性）

- `zoidbergbot.model.usage`
  - `zoidbergbot.channel`、`zoidbergbot.provider`、`zoidbergbot.model`
  - `zoidbergbot.sessionKey`、`zoidbergbot.sessionId`
  - `zoidbergbot.tokens.*`（input/output/cache_read/cache_write/total）
- `zoidbergbot.webhook.processed`
  - `zoidbergbot.channel`、`zoidbergbot.webhook`、`zoidbergbot.chatId`
- `zoidbergbot.webhook.error`
  - `zoidbergbot.channel`、`zoidbergbot.webhook`、`zoidbergbot.chatId`、`zoidbergbot.error`
- `zoidbergbot.message.processed`
  - `zoidbergbot.channel`、`zoidbergbot.outcome`、`zoidbergbot.chatId`、`zoidbergbot.messageId`、`zoidbergbot.sessionKey`、`zoidbergbot.sessionId`、`zoidbergbot.reason`
- `zoidbergbot.session.stuck`
  - `zoidbergbot.state`、`zoidbergbot.ageMs`、`zoidbergbot.queueDepth`、`zoidbergbot.sessionKey`、`zoidbergbot.sessionId`
...
- **Gateway网关不可达？** 先运行 `zoidbergbot doctor`。
