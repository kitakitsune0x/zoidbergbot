...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `zoidbergbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `zoidbergbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ```exec
   tail -f "${ZOIDBERGBOT_STATE_DIR:-$HOME/.openclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
