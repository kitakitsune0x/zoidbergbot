...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/zoidbergbot`)
...
If the page fails to load, start the Gateway first: `zoidbergbot gateway`.
...
- Config: view/edit `~/.zoidbergbot/zoidbergbot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ZoidbergBot
...
```bash
zoidbergbot gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
ZoidbergBot **blocks** Control UI connections without device identity.
...
```bash
ZOIDBERGBOT_CONTROL_UI_BASE_PATH=/zoidbergbot/ pnpm ui:build
```
