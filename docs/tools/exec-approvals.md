...
- **gateway host** → `zoidbergbot` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.zoidbergbot/exec-approvals.sock",
...
`~/.zoidbergbot/exec-approvals.json` directly.

CLI: `zoidbergbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
