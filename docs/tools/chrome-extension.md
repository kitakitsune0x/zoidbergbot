---
summary: "Chrome extension: let ZoidbergBot drive your existing Chrome tab"
read_when:
...
ZoidbergBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ZoidbergBot release (npm package) as static files. There is no separate “build” step.

After upgrading ZoidbergBot:

- Re-run `zoidbergbot browser extension install` to refresh the installed files under your ZoidbergBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ZoidbergBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ZoidbergBot to control.
...
- `ON`: attached; ZoidbergBot can drive that tab.
...
Debugging: `zoidbergbot sandbox explain`
...
`zoidbergbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `zoidbergbot browser extension install` first to copy the extension to a stable location under your ZoidbergBot state directory.
