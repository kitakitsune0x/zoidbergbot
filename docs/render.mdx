...
Deploy ZoidbergBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/zoidbergbot/zoidbergbot"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run ZoidbergBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ZOIDBERGBOT_STATE_DIR
        value: /data/.openclaw
      - key: ZOIDBERGBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: ZOIDBERGBOT_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: zoidbergbot-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/zoidbergbot`.
...
If you use the original ZoidbergBot repository, Render will not auto-deploy your ZoidbergBot. To update it, run a manual Blueprint sync from the dashboard.
...
For ZoidbergBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ZoidbergBot host.
