---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ZoidbergBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ZoidbergBot cron scheduling for periodic checks, or version status checks on a machine running ZoidbergBot (laptop, workstation, Pi, VPS).
---

# ZoidbergBot Host Hardening

## Overview

Assess and harden the host running ZoidbergBot, then align it to a user-defined risk tolerance without breaking access. Use ZoidbergBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ZoidbergBot changes the host firewall, SSH, or OS updates; it does not.
...
5. ZoidbergBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ZoidbergBot can access sensitive data.
...
### 2) Run ZoidbergBot security audits (read-only)

As part of the default read-only checks, run `zoidbergbot security audit --deep`. Only offer alternatives if the user requests them:

1. `zoidbergbot security audit` (faster, non-probing)
2. `zoidbergbot security audit --json` (structured output)

Offer to apply ZoidbergBot safe defaults (numbered):

1. `zoidbergbot security audit --fix`

Be explicit that `--fix` only tightens ZoidbergBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ZoidbergBot version/update status (read-only)

As part of the default read-only checks, run `zoidbergbot update status`.
...
- Credential hygiene notes (location of ZoidbergBot creds, prefer disk encryption)
...
- ZoidbergBot security audit (re-run)
...
After ZoidbergBot install or first hardening pass, run at least one baseline audit and version check:

- `zoidbergbot security audit`
- `zoidbergbot security audit --deep`
- `zoidbergbot update status`

Ongoing monitoring is recommended. Use the ZoidbergBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `zoidbergbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `zoidbergbot update status`

Also offer a periodic version check so the user can decide when to update (numbered):

1. `zoidbergbot update status` (preferred for source checkouts and channels)
2. `npm view zoidbergbot version` (published npm version)

## ZoidbergBot command accuracy

Use only supported commands and flags:

- `zoidbergbot security audit [--deep] [--fix] [--json]`
- `zoidbergbot status` / `zoidbergbot status --deep`
- `openclaw health --json`
- `zoidbergbot update status`
- `zoidbergbot cron add|list|runs|run`

Do not invent CLI flags or imply ZoidbergBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ZoidbergBot compaction:
