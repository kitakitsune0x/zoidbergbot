...
export function resolveHooksConfig(cfg: ZoidbergBotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-zoidbergbot-token"] === "string"
      ? req.headers["x-zoidbergbot-token"].trim()
      : "";
  if (headerToken) {
