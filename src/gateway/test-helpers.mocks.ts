...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `zoidbergbot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ZOIDBERGBOT_CONFIG_PATH = path.join(root, "zoidbergbot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "zoidbergbot.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "zoidbergbot-gateway-test"),
        ...fileDefaults,
