...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ZOIDBERGBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*zoidbergbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "zoidbergbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "zoidbergbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "zoidbergbot-gw");
  ...
  records.push(`_zoidbergbot-gw._tcp IN PTR ${instanceLabel}._zoidbergbot-gw._tcp`);
  records.push(`${instanceLabel}._zoidbergbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._zoidbergbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; zoidbergbot-content-hash: ${contentHash}\n${contentBody}`;
}
