...
import { loadZoidbergBotPlugins } from "../plugins/loader.js";
...
describe("tool_result_persist hook", () => {
  ...
  it("composes transforms in priority order and allows stripping toolResult.details", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "zoidbergbot-toolpersist-"));
    process.env.ZOIDBERGBOT_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
