...
function resolveExecConfig(cfg: ZoidbergBotConfig | undefined) {
...
export function createZoidbergBotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createZoidbergBotReadTool(freshReadTool)];
    }
