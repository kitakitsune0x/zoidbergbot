...
type WebSearchConfig = NonNullable<ZoidbergBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ZoidbergBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("zoidbergbot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.zoidbergbot.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://zoidbergbot.ai",
      "X-Title": "ZoidbergBot Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
