...
import { createZoidbergBotCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("zoidbergbot-ws-", async (workspaceDir) => {
      await withTempDir("zoidbergbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createZoidbergBotCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("zoidbergbot-ws-", async (workspaceDir) => {
      await withTempDir("zoidbergbot-override-", async (overrideDir) => {
        const tools = createZoidbergBotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("zoidbergbot-sandbox-", async (sandboxDir) => {
      await withTempDir("zoidbergbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "zoidbergbot-sbx-test",
        ...
        const tools = createZoidbergBotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
