...
describe("sessions", () => {
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-sessions-"));
  ...
  it("derives session transcripts dir from ZOIDBERGBOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ZOIDBERGBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.ZOIDBERGBOT_STATE_DIR;
    process.env.ZOIDBERGBOT_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.ZOIDBERGBOT_STATE_DIR = prev;
      }
    }
  });
