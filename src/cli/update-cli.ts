...
const STEP_LABELS: Record<string, string> = {
  ...
  "zoidbergbot doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "zoidbergbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const ZOIDBERGBOT_REPO_URL = "https://github.com/zoidbergbot/zoidbergbot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".zoidbergbot");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("zoidbergbot@")) {
    return trimmed.slice("zoidbergbot@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.ZOIDBERGBOT_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ZOIDBERGBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ZOIDBERGBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set ZOIDBERGBOT_GIT_DIR to an empty folder or an zoidbergbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ZOIDBERGBOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("ZoidbergBot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating ZoidbergBot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this ZoidbergBot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("zoidbergbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("zoidbergbot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g zoidbergbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g zoidbergbot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.ZOIDBERGBOT_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.ZOIDBERGBOT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("zoidbergbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("zoidbergbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("zoidbergbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("zoidbergbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `zoidbergbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `ZOIDBERGBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set ZOIDBERGBOT_GIT_DIR to an empty folder or an zoidbergbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via ZOIDBERGBOT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update ZoidbergBot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["zoidbergbot update", "Update a source checkout (git)"],
        ["zoidbergbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["zoidbergbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["zoidbergbot update --tag beta", "One-off update to a dist-tag or version"],
        ["zoidbergbot update --no-restart", "Update without restarting the service"],
        ["zoidbergbot update --json", "Output result as JSON"],
        ["zoidbergbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["zoidbergbot update wizard", "Interactive update wizard"],
        ["zoidbergbot --update", "Shorthand for zoidbergbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run zoidbergbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.zoidbergbot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zoidbergbot update status", "Show channel + version status."],
          ["zoidbergbot update status --json", "JSON output."],
          ["zoidbergbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.zoidbergbot.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
