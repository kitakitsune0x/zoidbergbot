...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `zoidbergbot-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.zoidbergbot/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@zoidbergbot/voice-call",
        version: "0.0.1",
        zoidbergbot: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without zoidbergbot.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@zoidbergbot/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("zoidbergbot.extensions");
  });
});
