...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    stateDir: process.env.ZOIDBERGBOT_STATE_DIR,
    configPath: process.env.ZOIDBERGBOT_CONFIG_PATH,
    skipChannels: process.env.ZOIDBERGBOT_SKIP_CHANNELS,
    skipGmail: process.env.ZOIDBERGBOT_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ZOIDBERGBOT_SKIP_CRON,
    skipCanvas: process.env.ZOIDBERGBOT_SKIP_CANVAS_HOST,
    skipBrowser: process.env.ZOIDBERGBOT_SKIP_BROWSER_CONTROL_SERVER,
    token: process.env.ZOIDBERGBOT_GATEWAY_TOKEN,
    password: process.env.ZOIDBERGBOT_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.ZOIDBERGBOT_STATE_DIR = stateDir;
    delete process.env.ZOIDBERGBOT_CONFIG_PATH;
    return stateDir;
  };

  beforeAll(async () => {
    process.env.ZOIDBERGBOT_SKIP_CHANNELS = "1";
    process.env.ZOIDBERGBOT_SKIP_GMAIL_WATCHER = "1";
    process.env.ZOIDBERGBOT_SKIP_CRON = "1";
    process.env.ZOIDBERGBOT_SKIP_CANVAS_HOST = "1";
    process.env.ZOIDBERGBOT_SKIP_BROWSER_CONTROL_SERVER = "1";
    delete process.env.ZOIDBERGBOT_GATEWAY_TOKEN;
    delete process.env.ZOIDBERGBOT_GATEWAY_PASSWORD;

    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.ZOIDBERGBOT_STATE_DIR = prev.stateDir;
    process.env.ZOIDBERGBOT_CONFIG_PATH = prev.configPath;
    process.env.ZOIDBERGBOT_SKIP_CHANNELS = prev.skipChannels;
    process.env.ZOIDBERGBOT_SKIP_GMAIL_WATCHER = prev.skipGmail;
    process.env.ZOIDBERGBOT_SKIP_CRON = prev.skipCron;
    process.env.ZOIDBERGBOT_SKIP_CANVAS_HOST = prev.skipCanvas;
    process.env.ZOIDBERGBOT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.ZOIDBERGBOT_GATEWAY_TOKEN = prev.token;
    process.env.ZOIDBERGBOT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "zoidbergbot");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.ZOIDBERGBOT_STATE_DIR = stateDir;
    process.env.ZOIDBERGBOT_CONFIG_PATH = path.join(stateDir, "zoidbergbot.json");
