...
<title>ZoidbergBot Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.zoidbergbotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.zoidbergbotCanvasA2UIAction &&
        typeof window.zoidbergbotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.zoidbergbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("zoidbergbot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ZoidbergBot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.zoidbergbotSendUserAction === "function"
        ? window.zoidbergbotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ZOIDBERGBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ZOIDBERGBOT_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".zoidbergbot", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ZoidbergBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
