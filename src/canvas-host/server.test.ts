...
describe("canvas host", () => {
  ...
  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-canvas-"));
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.zoidbergbotA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__zoidbergbot__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      ...
      expect(js).toContain("zoidbergbotA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
