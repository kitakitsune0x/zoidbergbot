...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "zoidbergbot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "zoidbergbot-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "zoidbergbot-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.zoidbergbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/zoidbergbot")).toBe(path.resolve(os.homedir(), "zoidbergbot"));
  });
