...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `zoidbergbot-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@zoidbergbot/zip-hooks",
        version: "0.0.1",
        zoidbergbot: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
      ...
      [
        ...
        'metadata: {"zoidbergbot":{"events":["command:new"]}}',
        "---",
        "",
        "# Zip Hook",
      ].join("\n"),
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@zoidbergbot/tar-hooks",
        version: "0.0.1",
        zoidbergbot: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with traversal-like ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/..",
        version: "0.0.1",
        zoidbergbot: { hooks: ["./hooks/evil-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with reserved ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/.",
        version: "0.0.1",
        zoidbergbot: { hooks: ["./hooks/reserved-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
