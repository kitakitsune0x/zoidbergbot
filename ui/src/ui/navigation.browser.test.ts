...
import { ZoidbergBotApp } from "./app";
import "../styles.css";

const originalConnect = ZoidbergBotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("zoidbergbot-app") as ZoidbergBotApp;
...
beforeEach(() => {
  ZoidbergBotApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__ZOIDBERGBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  ZoidbergBotApp.prototype.connect = originalConnect;
  window.__ZOIDBERGBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/zoidbergbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/zoidbergbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/zoidbergbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ZOIDBERGBOT_CONTROL_UI_BASE_PATH__ = "/zoidbergbot";
    const app = mountApp("/zoidbergbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/zoidbergbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/zoidbergbot/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "zoidbergbot.control.settings.v1",
