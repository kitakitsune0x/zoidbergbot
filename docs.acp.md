# ZoidbergBot ACP Bridge

This document describes how the ZoidbergBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`zoidbergbot acp` exposes an ACP agent over stdio and forwards prompts to a running
ZoidbergBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a ZoidbergBot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `zoidbergbot acp` over stdio.

Example config:

```bash
zoidbergbot config set gateway.remote.url wss://gateway-host:18789
zoidbergbot config set gateway.remote.token <token>
```

Example run:

```bash
zoidbergbot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
zoidbergbot acp --session agent:main:main
zoidbergbot acp --session agent:design:main
zoidbergbot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ZoidbergBot ACP” to start a thread.

## Execution Model

- ACP client spawns `zoidbergbot acp` and speaks ACP messages over stdio.
...
```bash
zoidbergbot acp --session agent:main:main
zoidbergbot acp --session-label "support inbox"
zoidbergbot acp --reset-session
```
...
`zoidbergbot acp` resolves the Gateway URL and auth from CLI flags or config:
